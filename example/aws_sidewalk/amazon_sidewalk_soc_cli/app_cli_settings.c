                                             ...
                                             const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    ...
    SL_SID_LOG_APP_ERROR("Error while trying to get link");
                                       ...
                                       const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  SL_SID_LOG_APP_ERROR("Attribute not supported for now.");
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    switch (settings.current_status.state) {
      case SID_STATE_READY:
        strcpy(s_state, "READY");
        break;

      case SID_STATE_NOT_READY:
        strcpy(s_state, "NOT_READY");
        break;

      case SID_STATE_ERROR:
        strcpy(s_state, "ERROR");
        break;

      case SID_STATE_SECURE_CHANNEL_READY:
        strcpy(s_state, "SECURE_CHANNEL_READY");
        break;
    ...
    SL_SID_LOG_APP_ERROR("Error while trying to get state");
    ret = SID_ERROR_GENERIC;
  }
                                     ...
                                     const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    ...
    snprintf(s_time, sizeof(s_time), "%u.%u", (unsigned)settings.current_time.tv_sec, (unsigned)settings.current_time.tv_nsec);
    sl_sidewalk_cli_util_set_string(s_time, key_str, entry);

    // Finally display value to user
    sl_sidewalk_cli_util_settings_get_string(value_str, key_str, entry);
  } else {
    SL_SID_LOG_APP_ERROR("Error while trying to get time");
  }
                                     ...
                                     const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  snprintf(s_rssi, sizeof(s_rssi), "%d", LAST_MESSG_RCVD_DESC.msg_desc_attr.rx_attr.rssi);
                                    ...
                                    const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  snprintf(s_snr, sizeof(s_snr), "%d", LAST_MESSG_RCVD_DESC.msg_desc_attr.rx_attr.snr);
                                                       ...
                                                       const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (!(strcmp(value_str, "no") == 0
        || strcmp(value_str, "ac") == 0
        || strcmp(value_str, "ml") == 0)) {
    SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for link connection policy: %s\n" \
    ...
    if ((strcmp(value_str, "ac")) == 0) {
      policy = SID_LINK_CONNECTION_POLICY_AUTO_CONNECT;
    } else if ((strcmp(value_str, "ml")) == 0) {
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    switch (settings.link_connection_policy) {
      case SID_LINK_CONNECTION_POLICY_AUTO_CONNECT:
        strcpy(s_link_connection_policy, "ac");
        break;

      case SID_LINK_CONNECTION_POLICY_MULTI_LINK_MANAGER:
        strcpy(s_link_connection_policy, "ml");
        break;

      default: // SID_LINK_CONNECTION_POLICY_NONE
        strcpy(s_link_connection_policy, "no");
        break;
    }
    ...
    SL_SID_LOG_APP_ERROR("Error while trying to get link connection policy");
    return SL_STATUS_FAIL;
  }
}
                                                  ...
                                                  const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  // Check if value wanted to be set is correct
  if (!(strcmp(value_str, "def") == 0
        || strcmp(value_str, "pow") == 0
        || strcmp(value_str, "per") == 0
        || strcmp(value_str, "lat") == 0
        || strcmp(value_str, "rel") == 0)) {
    SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for multi-link policy: %s\n" \
                                                    ...
                                                    const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  while (token != NULL) {
    if (token_no == 1) {
      ...
      } else if (!strcmp(token, "fsk")) {
        link_type_idx = 1;
        link_type = SID_LINK_TYPE_2;
      } else if (!strcmp(token, "css")) {
        link_type_idx = 2;
        link_type = SID_LINK_TYPE_3;
      } else {
        SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for auto connect link type parameter: %s\n" \
      ...
      if (!strcmp(token, "1")) {
        ...
        SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for auto connect link enabled parameter: %s\n" \
      ...
      if (priority == 0 && token == end_ptr) {
        SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for auto connect priority parameter: %s\n" \
      ...
      if (timeout == 0 && token == end_ptr) {
        SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for auto connect timeout parameter: %s\n" \
                             "Value must contain only numerical characters",
                             token);
        break;
      }
      ...
      SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for auto connect parameters: %s",
                           value_str);
      break;
    }
    token = strtok(NULL, ",");
  ...
  app_settings_sidewalk.auto_connect_params[link_type_idx].priority = (uint8_t)priority;
  app_settings_sidewalk.auto_connect_params[link_type_idx].connection_attempt_timeout_seconds = (uint16_t)timeout;
                                                    ...
                                                    const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  // Wait for data from sidewalk thread
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    snprintf(auto_connect_params,
             sizeof(auto_connect_params),
    ...
    SL_SID_LOG_APP_ERROR("Error while trying to get auto connect parameters");
    return SL_STATUS_FAIL;
  }
}
                                        ...
                                        const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    // Write result to time attribute
    snprintf(s_mtu, sizeof(s_mtu), "%u", settings.mtu);
    sl_sidewalk_cli_util_set_integer(s_mtu, key_str, entry);

    // Finally display value to user
    sl_sidewalk_cli_util_settings_get_integer(value_str, key_str, entry);
  } else {
    SL_SID_LOG_APP_ERROR("Error while trying to get MTU");
  }
                                           ...
                                           const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  switch (ble_config->config->mac_addr_type) {
    case SID_BLE_CFG_MAC_ADDRESS_TYPE_PUBLIC:
      strcpy(s_random_mac, "PUBLIC");
      break;

    case SID_BLE_CFG_MAC_ADDRESS_TYPE_RANDOM_PRIVATE_NON_RESOLVABLE:
      strcpy(s_random_mac, "RANDOM_PRIVATE_NON_RESOLVABLE");
      break;

    case SID_BLE_CFG_MAC_ADDRESS_TYPE_STATIC_RANDOM:
      strcpy(s_random_mac, "STATIC_RANDOM");
      break;

    case SID_BLE_CFG_MAC_ADDRESS_TYPE_RANDOM_PRIVATE_RESOLVABLE:
      strcpy(s_random_mac, "RANDOM_PRIVATE_RESOLVABLE");
      break;

    default:
      SL_SID_LOG_APP_ERROR("[CLI] Unsupported BLE_CFG for MAC address.");
      strcpy(s_random_mac, "UNKNOWN");
      break;
  }
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    switch (settings.device_profile.unicast_params.device_profile_id) {
      case SID_LINK2_PROFILE_1:
        strcpy(s_dev_prof_id, "1");
        break;

      case SID_LINK2_PROFILE_2:
        strcpy(s_dev_prof_id, "2");
        break;

      default:
        strcpy(s_dev_prof_id, "0");
        break;
    }
    ...
    SL_SID_LOG_APP_ERROR("Error while trying to get dev_profile_id");
  }
                                                  ...
                                                  const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  // Check if value wanted to be set is correct
  if (!(strcmp(value_str, "1") == 0 || strcmp(value_str, "2") == 0)) {
    SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for FSK power profile: %s\n" \
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    switch (settings.device_profile.unicast_params.device_profile_id) {
      case SID_LINK3_PROFILE_A:
        strcpy(s_dev_prof_id, "A");
        break;

      case SID_LINK3_PROFILE_B:
        strcpy(s_dev_prof_id, "B");
        break;

      case SID_LINK3_PROFILE_D:
        strcpy(s_dev_prof_id, "D");
        break;
                                                  ...
                                                  const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  // Check if value wanted to be set is correct
  if (!(strcmp(value_str, "A") == 0
        || strcmp(value_str, "B") == 0)) {
    SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for CSS power profile: %s\n" \
                                                    ...
                                                    const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  // Wait for data from sidewalk thread
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    snprintf(s_rx_window_count,
             sizeof(s_rx_window_count),
             "%d",
             settings.device_profile.unicast_params.rx_window_count);
                                                         ...
                                                         const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  // Wait for data from sidewalk thread
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    snprintf(s_rx_window_separation,
             sizeof(s_rx_window_separation),
             "%d",
            settings.device_profile.unicast_params.unicast_window_interval.sync_rx_interval_ms);
                                                         ...
                                                         const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (xQueueReceive(g_cli_event_queue, &settings, pdMS_TO_TICKS(2000))) {
    snprintf(s_rx_window_separation,
             sizeof(s_rx_window_separation),
             "%d",
             settings.device_profile.unicast_params.unicast_window_interval.async_rx_interval_ms);
                                                    ...
                                                    const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (!((sid_rx_window == SID_RX_WINDOW_CNT_2)
    ...
    SL_SID_LOG_APP_ERROR("Trying to set an incorrect value for \"CSS power profile A\" rx window count: %s\n"
                         "Correct values are: SID_RX_WINDOW_CNT_x, where x can be 2, 3, 4, 5'",
                         value_str);
    ret = SL_STATUS_FAIL;
  } else { // User has chosen a valid argument ...
    // Trigger sidewalk main thread for setting the stack parameters
    sl_app_trigger_sid_set_dev_prof_rx_win_cnt((int16_t)sid_rx_window);
