 ...
 * @returns None
 ******************************************************************************/
static void send(app_context_t *app_context,
 ...
 ******************************************************************************/
static uint8_t parse_link_type(const char *link_type_str);
 ...
 ******************************************************************************/
static int8_t parse_message_type(const char *message_type);
...
static size_t on_identifier_read(uint8_t *buf,
                               size_t len,
                               void *context);
static void on_motion_detection_event(const struct sid_motion_detection_event *event,
                                      void *context);
static void on_non_owner_find_event(const struct sid_non_owner_find_event *event,
                                    void *context);
static void on_owner_proximity_change(enum sid_owner_proximity_state state,
                                      void *context);
#endif
...
static const struct sid_detect_unwanted_location_tracker_accessory_info dult_info = {
  ...
  .capabilities = SID_DETECT_UNWANTED_DETECTION_TRACKER_CAPABILITY_SOUND
                  | SID_DETECT_UNWANTED_DETECTION_TRACKER_CAPABILITY_MOTION_DETECTION
                  | SID_DETECT_UNWANTED_DETECTION_TRACKER_CAPABILITY_IDENTIFIER_LOOKUP_BLE,
...
void main_task(void *context)
{
  ...
  g_event_queue = xQueueCreate(MSG_QUEUE_LEN,
                               sizeof(enum event_type));
  g_cli_event_queue = xQueueCreate(MSG_CLI_QUEUE_LEN,
                                   sizeof(app_setting_cli_queue_t));
  app_assert((g_event_queue != NULL) || (g_cli_event_queue != NULL),
      ...
      switch (event) {
        ...
        case EVENT_TYPE_SID_INIT:
          if (init_sidewalk(app_context, cli_arg_str) != SL_STATUS_OK) {
            SL_SID_LOG_APP_ERROR("sidewalk init failed");
          }
          break;
        ...
        case EVENT_TYPE_SID_DEINIT:
          if (deinit_sidewalk(app_context) != SL_STATUS_OK) {
            SL_SID_LOG_APP_ERROR("sidewalk deinit failed");
          }
          break;
        ...
        case EVENT_TYPE_DULT_INIT:
          if (init_dult(app_context) != SL_STATUS_OK) {
            SL_SID_LOG_APP_ERROR("dult init failed");
          }
          break;

        case EVENT_TYPE_DULT_DEINIT:
          if (deinit_dult(app_context) != SL_STATUS_OK) {
            SL_SID_LOG_APP_ERROR("dult deinit failed");
          }
          break;
#endif
        ...
        case EVENT_TYPE_SID_SET_DEV_PROF_RX_WIN_CNT:
          set_sidewalk_dev_prof_rx_win_cnt(app_context,
                                           (uint16_t)cli_arg_int16_t);
          break;
        ...
        case EVENT_TYPE_SID_SET_DEV_PROF_RX_INTERV_MS:
          set_sidewalk_dev_prof_rx_interv_ms(app_context,
                                             cli_arg_uint16_t);
          break;
        ...
        case EVENT_TYPE_SID_SET_DEV_PROF_WAKEUP_TYPE:
          set_sidewalk_dev_prof_wakeup_type(app_context,
                                            cli_arg_uint8_t);
          break;
        ...
        case EVENT_TYPE_SET_LINK_CONNECTION_POLICY:
          set_link_connection_policy(app_context,
                                     cli_arg_uint8_t);
          break;
        ...
        case EVENT_TYPE_SET_MULTI_LINK_POLICY:
          set_multi_link_policy(app_context,
                                cli_arg_uint8_t);
          break;
        ...
        case EVENT_TYPE_SET_AUTO_CONNECT_PARAMS:
          set_auto_connect_params(app_context,
                                  cli_arg_sid_link_auto_connect_params);
          break;
...
#endif
        default:
          SL_SID_LOG_APP_ERROR("unexpected event: %ld", event);
          break;
      }
    }
  }
...
{
  queue_event(g_event_queue,
              EVENT_TYPE_SID_OTA_DFU_RELEASE_BUFFER);
                                 ...
                                 void *context)
{
  ...
  SL_SID_LOG_APP_INFO("link type: %x, msg id: %u, msg type: %ld",
                      msg_desc->link_type,
                      msg_desc->id,
                      msg_desc->type);
}
                                   ...
                                   void *context)
{
  ...
  SL_SID_LOG_APP_ERROR("link type: %x, msg id: %u, msg type: %ld, error: %ld",
                       msg_desc->link_type,
                       msg_desc->id,
                       msg_desc->type,
                       error);
                                       ...
                                       void *context)
{
  ...
  switch (status->state) {
    ...
    case SID_STATE_ERROR:
      SL_SID_LOG_APP_ERROR("sidewalk status error, error: %ld",
...
static void connection_request(app_context_t *context)
{
  ...
  if (context->state == STATE_SIDEWALK_READY) {
    ...
    if (ret == SID_ERROR_NONE) {
      ...
      SL_SID_LOG_APP_ERROR("BLE connection request failed, error: %ld", ret);
    }
  }
#else
  (void)context;
...
static sl_status_t init_sidewalk(app_context_t *app_context, char *link_str)
{
  ...
  if (ret != SID_ERROR_NONE) {
    if (ret != SID_ERROR_ALREADY_INITIALIZED) {
      ...
      SL_SID_LOG_APP_ERROR("sidewalk init failed, error: %ld", ret);
    } else {
      // here we want to preserve the context sidewalk_handle from previous initialized link
      SL_SID_LOG_APP_ERROR("sidewalk already inited");
    }
    return SL_STATUS_FAIL;
  }
  SL_SID_LOG_APP_INFO("sidewalk inited");
...
static sl_status_t start_sidewalk(app_context_t *app_context, char *link_str)
{
  ...
  if (ret != SID_ERROR_NONE) {
    SL_SID_LOG_APP_ERROR("sidewalk start failed, link mask: %x, error: %ld",
                         link_mask, ret);
    app_context->sidewalk_handle = NULL;
    app_context->sid_cfg.link_mask = 0;
    return SL_STATUS_FAIL;
...
static sl_status_t stop_sidewalk(app_context_t *app_context, char *link_str)
{
  ...
  if (ret != SID_ERROR_NONE) {
    SL_SID_LOG_APP_ERROR("sidewalk stop failed, error: %ld", ret);
    app_context->sidewalk_handle = NULL;
    app_context->sid_cfg.link_mask = 0;
    return SL_STATUS_FAIL;
  } else {
...
static sl_status_t deinit_sidewalk(app_context_t *app_context)
{
  ...
  if (ret != SID_ERROR_NONE) {
    SL_SID_LOG_APP_ERROR("sidewalk deinit failed, error: %ld", ret);
    app_context->sidewalk_handle = NULL;
    app_context->sid_cfg.link_mask = 0;
    return SL_STATUS_FAIL;
  } else {
    SL_SID_LOG_APP_INFO("sidewalk deinited");
  }
...
static void reset_sidewalk(app_context_t *context)
{
  ...
  if (context->state == STATE_SIDEWALK_READY) {
    ...
    if (ret != SID_ERROR_NONE) {
      SL_SID_LOG_APP_ERROR("factory reset failed, error: %ld", ret);
...
static sl_status_t init_dult(app_context_t *context)
{
  ...
  ret = sid_detect_unwanted_location_tracker_init(&config,
                                                  context->sidewalk_handle);
  if (ret == SID_ERROR_ALREADY_INITIALIZED) {
    SL_SID_LOG_APP_ERROR("dult already initialized");
  } else if (ret == SID_ERROR_INVALID_STATE) { // Not reset sidewalk handle if 'dult init' is issued during sidewalk started state
    SL_SID_LOG_APP_ERROR("dult cannot be initialized during sidewalk started state, error: %ld", ret);
  } else if (ret != SID_ERROR_NONE) {
    SL_SID_LOG_APP_ERROR("dult initialization failed, error: %ld", ret);
...
static sl_status_t deinit_dult(app_context_t *context)
{
  ...
  ret = sid_detect_unwanted_location_tracker_deinit(context->sidewalk_handle);
  ...
  if (ret == SID_ERROR_INVALID_STATE) {
    ...
    SL_SID_LOG_APP_ERROR("dult cannot be deinitialized during sidewalk started state, error: %ld", ret);
  } else if (ret != SID_ERROR_NONE) {
    SL_SID_LOG_APP_ERROR("dult deinitialization failed, error: %ld", ret);
    context->sidewalk_handle = NULL;
    context->sid_cfg.link_mask = 0;
  } else {
