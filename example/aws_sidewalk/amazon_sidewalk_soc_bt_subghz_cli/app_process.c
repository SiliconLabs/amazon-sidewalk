...
static void log_sid_msg(const struct sid_msg *msg)
{
  app_log_hexdump_info(msg->data, msg->size);
}
...
{
  UNUSED(context);
  app_log_info("App - sent message (type: %d, id: %u)", msg_desc->type, msg_desc->id);
}
...
static void on_sidewalk_status_changed(const struct sid_status *status, void *context)
{
  ...
  switch (status->state) {
    ...
    case SID_STATE_ERROR:
      app_log_error("App - sidewalk state error: %d", sid_get_error(app_context->sidewalk_handle));
      break;
...
static sl_status_t deinit_sidewalk(app_context_t *app_context)
{
  ...
  if (ret != SID_ERROR_NONE) {
    app_log_error("App - failed to deinitialise sidewalk stack: %d\n", ret);
    app_context->sidewalk_handle = NULL;
    app_context->sid_cfg.link_mask = 0;

    return SL_STATUS_FAIL;
  } else {
    app_log_info("App - sidewalk stack deinitialised\n");
  }
