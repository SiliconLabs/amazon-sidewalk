...
static uint8_t connection_id = 0U;
...
static sl_status_t update_report_characteristic(uint16_t attribute, uint8_t data_send)
{
  ...
  sc = sl_bt_gatt_server_write_attribute_value(attribute,
                                               ...
                                               &data_send);
  if (sc == SL_STATUS_OK) {
    SL_SID_LOG_APP_INFO("BLE btn characteristic updated, attr: %d, data: 0x%02x",
                        attribute, data_send);
  }
...
static sl_status_t send_report_notification(uint16_t attribute)
{
  ...
  sc = sl_bt_gatt_server_send_notification(connection_id,
                                            attribute,
                                            sizeof(data_send),
                                            &data_send);
  if (sc == SL_STATUS_OK) {
    SL_SID_LOG_APP_INFO("BLE report notif sent, attr: %d, data: 0x%02x",
                        attribute, data_send);
  }
  return sc;
}
