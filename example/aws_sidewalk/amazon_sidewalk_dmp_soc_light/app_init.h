...
enum event_type{
  ...
  EVENT_TYPE_TOGGLE_LED_SOURCE_BLE,
  EVENT_TYPE_TOGGLE_LED_SOURCE_CLOUD,
  EVENT_TYPE_TOGGLE_LED_SOURCE_BOARD,
  EVENT_TYPE_BTN_PRESS,
  EVENT_TYPE_BTN_PRESS_SEND_RESP,
  EVENT_TYPE_INVALID
};

enum toggle_led_source{
  TOGGLE_LED_SOURCE_BLE = 0,
  TOGGLE_LED_SOURCE_CLOUD = 1,
  TOGGLE_LED_SOURCE_BOARD = 2,
};
...
typedef struct app_led_status{
  uint8_t value;
  enum toggle_led_source source;
} app_led_status_t;
...
typedef struct app_context{
  ...
  SemaphoreHandle_t led_status_semaphore;
