...
enum event_type{
  ...
  EVENT_TYPE_LOCATION_SEND_L1,
  EVENT_TYPE_LOCATION_SEND_L3,
  EVENT_TYPE_LOCATION_SEND_L4,
#if defined(SL_LOCATION_FULL)
  EVENT_TYPE_LOCATION_SEND_BUF_L3,
  EVENT_TYPE_LOCATION_SEND_BUF_L4,
  EVENT_TYPE_LOCATION_SCAN_L3,
  EVENT_TYPE_LOCATION_SCAN_L4,
#endif
 ...
 ******************************************************************************/
static void location_send(enum sid_location_effort_mode effort);
 ...
 * @param[in] effort Requested effort level
 ******************************************************************************/
static void location_send_buf(enum sid_location_effort_mode effort);
 ...
 * @param[in] effort Requested effort level
 ******************************************************************************/
static void location_scan(enum sid_location_effort_mode effort);
#endif
  ...
  while (1) {
    ...
    if (xQueueReceive(app_event_queue, &event, portMAX_DELAY) == pdTRUE) {
      ...
      switch (event) {
        ...
        case EVENT_TYPE_LOCATION_SEND_L1:
          SL_SID_LOG_APP_INFO("location send L1 event");
          location_send(SID_LOCATION_EFFORT_L1);
          break;

        case EVENT_TYPE_LOCATION_SEND_L3:
          SL_SID_LOG_APP_INFO("location send L3 event");
          location_send(SID_LOCATION_EFFORT_L3);
          break;

        case EVENT_TYPE_LOCATION_SEND_L4:
          SL_SID_LOG_APP_INFO("location send L4 event");
          location_send(SID_LOCATION_EFFORT_L4);
          break;

#if defined(SL_LOCATION_FULL)
        case EVENT_TYPE_LOCATION_SEND_BUF_L3:
          SL_SID_LOG_APP_INFO("location send buf L3 event");
          location_send_buf(SID_LOCATION_EFFORT_L3);
          break;

        case EVENT_TYPE_LOCATION_SEND_BUF_L4:
          SL_SID_LOG_APP_INFO("location send buf L4 event");
          location_send_buf(SID_LOCATION_EFFORT_L4);
          break;

        case EVENT_TYPE_LOCATION_SCAN_L3:
          SL_SID_LOG_APP_INFO("location scan L3 event");
          location_scan(SID_LOCATION_EFFORT_L3);
          break;

        case EVENT_TYPE_LOCATION_SCAN_L4:
          SL_SID_LOG_APP_INFO("location scan L4 event");
          location_scan(SID_LOCATION_EFFORT_L4);
          break;
#endif
...
static void location_send(enum sid_location_effort_mode effort)
{
  ...
  SL_SID_LOG_APP_INFO("location send result: %d", res);
}
...
static void location_send_buf(enum sid_location_effort_mode effort)
{
  ...
  sid_error_t res = sid_location_run(application_context.sidewalk_handle, &config, 0);
  SL_SID_LOG_APP_INFO("location send buf result: %d", res);
}
...
static void location_scan(enum sid_location_effort_mode effort)
{
  ...
  sid_error_t res = sid_location_run(application_context.sidewalk_handle, &config, 0);
  SL_SID_LOG_APP_INFO("location scan result: %d", res);
}
...
void app_trigger_location_send(enum sid_location_effort_mode effort)
{
  switch (effort) {
    case SID_LOCATION_EFFORT_L1:
      queue_event(app_event_queue, EVENT_TYPE_LOCATION_SEND_L1);
      break;
    case SID_LOCATION_EFFORT_L3:
      queue_event(app_event_queue, EVENT_TYPE_LOCATION_SEND_L3);
      break;
    case SID_LOCATION_EFFORT_L4:
      queue_event(app_event_queue, EVENT_TYPE_LOCATION_SEND_L4);
      break;
    default:
      SL_SID_LOG_APP_ERROR("location send invalid argument: %d", effort);
      break;
  }
}
...
void app_trigger_location_send_buf(enum sid_location_effort_mode effort)
{
  switch (effort) {
    case SID_LOCATION_EFFORT_L3:
      queue_event(app_event_queue, EVENT_TYPE_LOCATION_SEND_BUF_L3);
      break;
    case SID_LOCATION_EFFORT_L4:
      queue_event(app_event_queue, EVENT_TYPE_LOCATION_SEND_BUF_L4);
      break;
    default:
      SL_SID_LOG_APP_ERROR("location send buf invalid argument: %d", effort);
      break;
  }
}
...
void app_trigger_location_scan(enum sid_location_effort_mode effort)
{
  switch (effort) {
    case SID_LOCATION_EFFORT_L3:
      queue_event(app_event_queue, EVENT_TYPE_LOCATION_SCAN_L3);
      break;
    case SID_LOCATION_EFFORT_L4:
      queue_event(app_event_queue, EVENT_TYPE_LOCATION_SCAN_L4);
      break;
    default:
      SL_SID_LOG_APP_ERROR("location scan invalid argument: %d", effort);
      break;
  }
}
#endif
