...
void app_init(void)
{
  // Initialize Silabs system
  sl_system_init();

  app_log_info("App - sid_ble application started");
  ...
  // Initialize PAL Key-Value storage
  ...
  BaseType_t status = xTaskCreate(main_thread, "MAIN", MAIN_TASK_STACK_SIZE, &app_context, 1, &app_context.main_task);
