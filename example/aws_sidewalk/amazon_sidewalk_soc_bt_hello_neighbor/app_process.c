 ...
 * @param[in] context The context which is applicable for the current application
 ******************************************************************************/
static void get_connection_status(app_context_t *context);
...
{
  queue_event(g_event_queue, EVENT_TYPE_GET_CONNECTION_STATUS, true);

  app_log_info("App - get connection status user event");
}
...
void main_thread(void *context)
{
  ...
  if (ret != SID_ERROR_NONE) {
    app_log_error("App - failed to initialize sidewalk: %d", (int)ret);
    goto error;
  }
  //Register sidewalk handler to the application context
  app_context->sidewalk_handle = sid_handle;

  //Start the sidewalk stack
  ret = sid_start(sid_handle, SID_LINK_TYPE_1);
  if (ret != SID_ERROR_NONE) {
    app_log_error("App - failed to start sidewalk: %d", (int)ret);
    goto error;
  }
  ...
  while (1) {
    ...
    if (xQueueReceive(app_context->event_queue, &event, portMAX_DELAY) == pdTRUE) {
      ...
      switch (event) {
        ...
        case EVENT_TYPE_GET_CONNECTION_STATUS:
          app_log_info("App - get connection status event");

          get_connection_status(app_context);
          break;
...
{
  if (context->state == STATE_SIDEWALK_READY
      || context->state == STATE_SIDEWALK_SECURE_CONNECTION) {
    app_log_info("App - conn status: connected");
  } else {
    app_log_info("App - conn status: not connected");
  }
}
...
{
  size_t mtu;
  sid_error_t ret = sid_get_mtu(context->sidewalk_handle, SID_LINK_TYPE_1, &mtu);
  if (ret == SID_ERROR_NONE) {
