...
## Migration to Sidewalk v2.8.1

Starting with Sidewalk v2.8.1, this application supports the Connected Device demo app. Despite the upgrade, projects migrating from Sidewalk v2.8.0 or earlier will continue to use the Blinky demo app.

To resolve build errors that may occur after upgrading your project, apply the following modifications to the LED Manager API calls in `app_process.c`:

1. **`sl_sidewalk_led_manager_toggle_led`**: Provide a second parameter of type `sl_sidewalk_led_manager_source_t`. This parameter is not used in the Blinky demo app; a value of `0` is recommended.
2. **`sl_sidewalk_led_manager_led_state_changed`**: Provide a third parameter of type `sl_sidewalk_led_manager_source_t source`. This parameter is not used in the Blinky demo app.

**Example — `sl_sidewalk_led_manager_toggle_led` call:**

```c
// Before (Sidewalk v2.8.0 and earlier):
sl_sidewalk_led_manager_toggle_led(0);

// After (Sidewalk v2.8.1):
sl_sidewalk_led_manager_toggle_led(0, 0);
```

**Example — `sl_sidewalk_led_manager_led_state_changed` callback signature:**
