...
def generate_properties(f):
    ...
    add_property(
        "SL_SIDEWALK_CONFIGURATOR_PROPERTIES_FSK_POWER_PROFILE", propFskPowerProfile
    )
    # output power
    propFskOutputPower = find_in_json_file("fskSettings", "outputPower", file=f)
    add_property(
        "SL_SIDEWALK_CONFIGURATOR_PROPERTIES_FSK_OUTPUT_POWER", propFskOutputPower
    )
    ...
    add_property(
        "SL_SIDEWALK_CONFIGURATOR_PROPERTIES_CSS_POWER_PROFILE", propCSSPowerProfile
    )
    # output power
    propCSSOutputPower = find_in_json_file("cssSettings", "outputPower", file=f)
    add_property(
        "SL_SIDEWALK_CONFIGURATOR_PROPERTIES_CSS_OUTPUT_POWER", propCSSOutputPower
    )
...
def parse_restrictions_cproject():
    ...
    # defaultLinkType
    defaultLink = c_header_parser(
        COMMON_CONFIG_PATH, "SL_SIDEWALK_COMMON_DEFAULT_LINK_TYPE"
    )
    if defaultLink:
...
def generate_restrictions():
    ...
    if os.path.exists(
        os.path.join(PROJECT_ROOT_PATH, "config/sidewalk/link_sidewalk.restriction")
    ) and os.path.exists(
        os.path.join(PROJECT_ROOT_PATH, "config/sidewalk/region_sidewalk.restriction")
    ):
        return
