...
int sid_pal_radio_hold_scan()
{
    ...
    if( sid_pal_gpio_read( drv_ctx.config->gpios.int1, &pinState ) == SID_ERROR_NONE ) {
		if (pinState) {
			SL_SID_LOG_APP_WARNING("hold scan: int1 high");
			return -1;
		}
	} else {
...
#endif /* CONFIG_RADIO_LOCK_TEST */
     taskEXIT_CRITICAL();
     SL_SID_LOG_APP_WARNING("hold scan: radio in use");
     return -1;
  }
...
void sid_pal_radio_release_scan()
{
	...
	if (drv_ctx.radio_state != SID_PAL_RADIO_SCAN) {
		taskEXIT_CRITICAL();
		SL_SID_LOG_APP_WARNING("release scan: not in scan mode");
		return;
	}
	taskEXIT_CRITICAL();
...
static int32_t radio_lr11xx_platform_init( void )
{
    ...
    drv_ctx.hal_lock = xSemaphoreCreateBinaryStatic(&sem_buffer);
    xSemaphoreGive(drv_ctx.hal_lock);
