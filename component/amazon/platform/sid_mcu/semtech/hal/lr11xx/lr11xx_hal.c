...
lr11xx_hal_status_t lr11xx_hal_abort_blocking_cmd( const void* context )
{
  ...
  if (err != SID_ERROR_NONE) {
    SL_SID_LOG_APP_ERROR("LR11xx: abort_blocking xfer");
    return LR11XX_HAL_STATUS_ERROR;
  }

  if (lr11xx_wait_on_busy(drv_ctx, 1000000) != SID_ERROR_NONE) {// microseconds 1,000,000
    SL_SID_LOG_APP_ERROR("LR11xx: abort_blocking wait on busy");
    return LR11XX_HAL_STATUS_ERROR;
  } else
...
lr11xx_hal_status_t lr11xx_hal_direct_read( const void* context, uint8_t* data, const uint16_t data_length, const char *caller )
{
    ...
    if (drv_ctx->sleeping) {
      ...
      SL_SID_LOG_APP_WARNING("LR11xx: direct_read self-wakeup");
      if ((err = lr11xx_hal_wakeup(drv_ctx)) != LR11XX_HAL_STATUS_OK) {
        SL_SID_LOG_APP_ERROR("LR11xx: fail wakeup %d", err);
        return LR11XX_HAL_STATUS_ERROR;
      }
    }
    ...
    {
        SL_SID_LOG_APP_ERROR("LR11xx: direct_read %s wait_on_busy, last %04x", caller, drv_ctx->last.command);
        return LR11XX_HAL_STATUS_ERROR;
    }

    int err = drv_ctx->bus_iface->xfer( drv_ctx->bus_iface, &config->bus_selector, empty, data, data_length );
    if (err != SID_ERROR_NONE) {
        SL_SID_LOG_APP_ERROR("LR11xx: direct_read xfer fail %d", err);
    }
...
uint8_t _read_busy(const halo_drv_semtech_ctx_t* drv_ctx)
{
  ...
  if (err != SID_ERROR_NONE)
    SL_SID_LOG_APP_ERROR("LR11xx: read busy pin fail");
...
lr11xx_hal_status_t lr11xx_hal_wakeup( const void* context )
{
    ...
    if (drv_ctx->radio_state != SID_PAL_RADIO_SLEEP && !drv_ctx->sleeping) {
      SL_SID_LOG_APP_WARNING("LR11xx: !SID_PAL_RADIO_SLEEP and !sleeping");
      return LR11XX_HAL_STATUS_OK;
    }

    if (!_read_busy(drv_ctx)) {
      SL_SID_LOG_APP_WARNING("LR11xx: hal_wakeup busy low");
    }
    ...
    if (drv_ctx->config->wakeup_delay_us != 0) {
      if (drv_ctx->config->wakeup_delay_us < 95) {
        SL_SID_LOG_APP_ERROR("LR11xx: wakeup_delay_us too low, %u", drv_ctx->config->wakeup_delay_us);
      } else {
        SL_SID_LOG_APP_INFO("LR11xx: wakeup_delay_us %u", drv_ctx->config->wakeup_delay_us);
      }
    }
                                      ...
                                      const uint8_t* data, const uint16_t data_length )
{
    ...
    {
        SL_SID_LOG_APP_ERROR("LR11xx: hal_write dropped ctx %p, cmd %p, cmd_len %u", context, command, command_length);
        return LR11XX_HAL_STATUS_ERROR;
    }
    ...
    if (ret == LR11XX_HAL_STATUS_OK && is_scan_start_command(command)) {
      ...
      if (cctx->radio_state != SID_PAL_RADIO_SCAN) {
        ...
        SL_SID_LOG_APP_ERROR("LR11xx: scan start cmd while not in scan state");
      }
    }
    return ret;
}
                                            ...
                                            bool read )
{
    ...
    {
        SL_SID_LOG_APP_ERROR("LR11xx: rdwr start wait_on_busy 0x%02X%02X, waited %u", command[0], command[1], max_wait_us );
        return LR11XX_HAL_STATUS_ERROR;
    }
    ...
    if( err != SID_ERROR_NONE )
    {
        SL_SID_LOG_APP_ERROR("LR11xx: rdwr write xfer fail");
        return LR11XX_HAL_STATUS_ERROR;
    }
    ...
    if( !( drv_ctx->last.stat1 & STATUS_OK_MASK ) && drv_ctx->last.command )
    {
		...
		if (drv_ctx->aborted) {
			if ((drv_ctx->last.stat1 & 0x0e) == (LR11XX_SYSTEM_CMD_STATUS_CMD_ABORT << STATUS_FIELD_OFFSET_BITS)) {
				SL_SID_LOG_APP_INFO("LR11xx: abort response: CMD_ABORT");
				ok = true;
			} else {
				SL_SID_LOG_APP_INFO("LR11xx: abort response, stat1 %x", drv_ctx->last.stat1);
			}
			drv_ctx->aborted = false;
		}
		if (!ok) {
			/* section 3.4.2: bit0 = interrupt status */
			SL_SID_LOG_APP_WARNING("LR11xx: during 0x%02X%02X, Command 0x%.4X failed; Stat1 0x%.2X int1:%d Stat2 0x%.2X ", command[0], command[1],
    ...
    if( lr11xx_wait_on_busy( drv_ctx, SEMTECH_MAX_WAIT_ON_BUSY_CNT_US ) != SID_ERROR_NONE )
    {
        SL_SID_LOG_APP_ERROR("LR11xx: rdwr read busy");
        return LR11XX_HAL_STATUS_ERROR;
    }
    ...
    if( err != SID_ERROR_NONE )
    {
        SL_SID_LOG_APP_ERROR("LR11xx: rdwr read xfer fail");
        return LR11XX_HAL_STATUS_ERROR;
    }
    ...
    if( !( drv_ctx->last.stat1 & STATUS_OK_MASK ) && drv_ctx->last.command )
    {
        SL_SID_LOG_APP_WARNING("LR11xx: Command rsp 0x%.4X failed; Stat1 0x%.2X Stat2 0x%.2X", drv_ctx->last.command, drv_ctx->last.stat1, drv_ctx->last.stat2);
    }
