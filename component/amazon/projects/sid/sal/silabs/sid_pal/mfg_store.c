...
#include "sl_hal_system.h" // for sl_hal_system_get_unique
...
//                                Static Variables
// -----------------------------------------------------------------------------
static const uint32_t MFG_WORD_SIZE = 4U; // in bytes
// -----------------------------------------------------------------------------
...
void sid_pal_mfg_store_init(sid_pal_mfg_store_region_t mfg_store_region)
{
  ...
  if (!nvm3_defaultHandle->hasBeenOpened) {
    ...
    if (ECODE_NVM3_OK != status) {
      SL_SID_LOG_PAL_ERROR("pal mfg: mfg store init err: %u", (unsigned)status);
      return;
    }
  }

  uint16_t obj_cnt = (uint16_t)nvm3_enumObjects(nvm3_defaultHandle,
                                                NULL,
                                                0,
...
int32_t sid_pal_mfg_store_write(uint16_t value, const uint8_t *buffer, uint16_t length)
{
  ...
  if (!SLI_SID_NVM3_VALIDATE_KEY(MFG, value)) {
    SL_SID_LOG_PAL_ERROR("pal mfg: mfg write, key 0x%.5x not in range (0x%.5x - 0x%.5x)",
                         value, SLI_SID_NVM3_KEY_MIN_MFG_REL, SLI_SID_NVM3_KEY_MAX_MFG_REL);
  ...
  Ecode_t status = nvm3_writeData(nvm3_defaultHandle,
                                  SLI_SID_NVM3_MAP_KEY(MFG, value),
                                  buffer,
                                  (size_t)length);
  if (status != ECODE_NVM3_OK) {
    SL_SID_LOG_PAL_ERROR("pal mfg: mfg write, write err: %u", (unsigned)status);
  ...
  if (nvm3_repackNeeded(nvm3_defaultHandle)) {
    ...
    if (status != ECODE_NVM3_OK) {
      SL_SID_LOG_PAL_ERROR("pal mfg: mfg write, repack err: %u", (unsigned)status);
...
void sid_pal_mfg_store_read(uint16_t value, uint8_t *buffer, uint16_t length)
{
  ...
  if (!SLI_SID_NVM3_VALIDATE_KEY(MFG, value)) {
    SL_SID_LOG_PAL_ERROR("pal mfg: mfg read, key 0x%.5x not in range (0x%.5x - 0x%.5x)",
                         value, SLI_SID_NVM3_KEY_MIN_MFG_REL, SLI_SID_NVM3_KEY_MAX_MFG_REL);
    return;
  }
...
uint16_t sid_pal_mfg_store_get_length_for_value(uint16_t value)
{
  ...
  if (!SLI_SID_NVM3_VALIDATE_KEY(MFG, value)) {
    SL_SID_LOG_PAL_ERROR("pal mfg: mfg get len for value, key 0x%.5x not in range (0x%.5x - 0x%.5x)",
                         value, SLI_SID_NVM3_KEY_MIN_MFG_REL, SLI_SID_NVM3_KEY_MAX_MFG_REL);
    return (uint16_t)object_length;
  }
  ...
  return (object_type == NVM3_OBJECTTYPE_DATA) ? (uint16_t)object_length : 0U;
}
...
int32_t sid_pal_mfg_store_erase(void)
{
  ...
  obj_cnt = nvm3_enumObjects(nvm3_defaultHandle,
                             NULL,
                             0,
                             SLI_SID_NVM3_KEY_MIN_MFG,
                             SLI_SID_NVM3_KEY_MAX_MFG);
  ...
  nvm3_enumObjects(nvm3_defaultHandle,
                   key_list,
                   obj_cnt,
                   SLI_SID_NVM3_KEY_MIN_MFG,
                   SLI_SID_NVM3_KEY_MAX_MFG);
  for (uint32_t i = 0; i < obj_cnt; i++) {
    ...
    if (ECODE_NVM3_OK != status) {
      SL_SID_LOG_PAL_ERROR("pal mfg: mfg erase, erase err: %u", (unsigned)status);
      sl_free(key_list);
...
bool sid_pal_mfg_store_dev_id_get(uint8_t dev_id[SID_PAL_MFG_STORE_DEVID_SIZE])
{
  ...
  if (memcmp(buffer, unset_dev_id, SID_PAL_MFG_STORE_DEVID_SIZE) == 0) {
    ...
    if ((MFG_VERSION_1_VAL == version) || (0x1 == version)) {
      ...
      uint32_t val = 0;
      sid_pal_mfg_store_read(SID_PAL_MFG_STORE_DEVID, dev_id_buffer, (uint16_t)sizeof(dev_id_buffer));
