                                          ...
                                          uint32_t len)
{
  ...
  // Check if record exists using record_get_len to avoid size mismatch issues
  // when updating a record with different size
  uint32_t existing_len = 0;
  sid_error_t sid_ret = sid_pal_storage_kv_record_get_len(group, key, &existing_len);

  if (sid_ret == SID_ERROR_NONE) {
    // Record exists - check if data is the same
    if (existing_len == len) {
      static uint8_t read_buf[SID_PAL_KV_STORE_MAX_LENGTH_BYTES];
      memset(read_buf, 0, sizeof(read_buf));
      sid_error_t read_ret = sid_pal_storage_kv_record_get(group, key, read_buf, len);
      if (read_ret == SID_ERROR_NONE && memcmp(read_buf, p_data, len) == 0) {
        return SID_ERROR_NONE;
      }
    }
    // Record with the same key exists (may have different size/data)
