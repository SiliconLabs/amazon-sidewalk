...
{
  (void)context;
  for (uint8_t ix = 0U; ix < SL_PIN_MAX; ++ix) {
...
static void sid_pal_pin_mode_input(const struct GPIO_LookupItem *lookupptr,
                                   sl_gpio_mode_t *mode,
                                   bool *out)
{
  if (!lookupptr || !mode || !out) {
    return;
  }
...
static void sid_pal_pin_mode_output(const struct GPIO_LookupItem *lookupptr,
                                    sl_gpio_mode_t *mode,
                                    bool *out)
{
  if (!lookupptr || !mode || !out) {
...
{
  struct GPIO_LookupItem *lookupptr = NULL;
  ...
  return sid_pal_pin_mode_set(lookupptr);
}
...
sid_error_t sid_pal_gpio_read(uint32_t gpio_number,
                              uint8_t *value)
{
  const struct GPIO_LookupItem *lookupptr = NULL;
  ...
  lookupptr = &gpio_lookup_table[gpio_number];
  if (sl_gpio_get_pin_input(&lookupptr->gpio,
                            &pin_value) != SL_STATUS_OK) {
    return SID_ERROR_GENERIC;
  }

  *value = pin_value ? 1U : 0U;
  return SID_ERROR_NONE;
}
                                 ...
                                 void *callback_arg)
{
  ...
  if (lookupptr->IntNO != SL_GPIO_INTERRUPT_UNAVAILABLE) {
    if (sl_gpio_deconfigure_external_interrupt(lookupptr->IntNO) != SL_STATUS_OK) {
      return SID_ERROR_GENERIC;
    }
    lookupptr->IntNO = SL_GPIO_INTERRUPT_UNAVAILABLE;
  }
  if (sl_gpio_configure_external_interrupt(&lookupptr->gpio,
                                           &lookupptr->IntNO,
                                           flags,
                                           gpio_irq_handler,
                                           NULL) != SL_STATUS_OK) {
