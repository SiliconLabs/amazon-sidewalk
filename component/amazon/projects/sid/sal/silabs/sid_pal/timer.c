                              ...
                              const struct sid_timespec *period)
{
  ...
  if (sl_sleeptimer_start_timer(&(timer->sleeptimer_handle),
                                (uint32_t)timeout_tick,
                                sleeptimer_callback,
                                timer,
                                (type == SID_PAL_TIMER_PRIO_CLASS_LOWPOWER) ? 10U : 0U,
                                0) != SID_ERROR_NONE) {
                                ...
                                void *data)
{
  ...
  timer = (sid_pal_timer_t *)data;
  if (timer->is_periodic && !timer->has_started) {
    timer->has_started = true;
    ret = sl_sleeptimer_start_periodic_timer_ms(&(timer->sleeptimer_handle),
                                                timer->period_in_ms,
                                                sleeptimer_callback,
                                                timer,
                                                0,
