...
static int32_t radio_efr32xgxx_get_tx_power_range(int8_t *max_tx_power,
                                                  int8_t *min_tx_power);
...
int32_t sid_pal_radio_set_frequency(uint32_t freq)
{
  ...
  if ((drv_ctx.regional_radio_param.param_region == RADIO_REGION_NA)
      && ((freq < SIDEWALK_FSK_US_START_FREQUENCY) || (freq > SIDEWALK_FSK_US_END_FREQUENCY))) {
    //invalid frequency
...
int32_t sid_pal_radio_get_max_tx_power(sid_pal_radio_data_rate_t data_rate, int8_t *tx_power)
{
  ...
  if (radio_efr32xgxx_get_tx_power_range(&radio_max, &radio_min) != RADIO_ERROR_NONE) {
    err = RADIO_ERROR_HARDWARE_ERROR;
    goto ret;
  }
  (void) radio_min;
...
int32_t sid_pal_radio_set_tx_power(int8_t power)
{
  ...
  if (radio_efr32xgxx_get_tx_power_range(&max, &min) != RADIO_ERROR_NONE) {
    err = RADIO_ERROR_HARDWARE_ERROR;
    goto ret;
  }
  if (power < min) {
...
int32_t sid_pal_radio_is_channel_free(uint32_t freq,
                                      int16_t threshold,
                                      uint32_t delay_us,
                                      bool *is_channel_free)
{
  ...
  *is_channel_free = false;
  ...
  if (delay_us < EFR32XGXX_MIN_CHANNEL_FREE_DELAY_US) {
    SL_SID_LOG_PAL_WARNING("pal rail: min ch free delay raised (req: %ld, min: %ld)",
                           delay_us,
                           EFR32XGXX_MIN_CHANNEL_FREE_DELAY_US);
    delay_us = EFR32XGXX_MIN_CHANNEL_FREE_DELAY_US;
  }

  struct sid_timespec t_start;
  struct sid_timespec t_cur;
  struct sid_timespec t_threshold;
...
static int32_t radio_efr32xgxx_get_tx_power_range(int8_t *max_tx_power, int8_t *min_tx_power)
{
  if (max_tx_power == NULL || min_tx_power == NULL) {
    return RADIO_ERROR_INVALID_PARAMS;
  }
