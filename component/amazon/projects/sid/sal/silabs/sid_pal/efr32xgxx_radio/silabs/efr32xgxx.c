...
void efr32xgxx_get_txpower(int8_t *max_tx_power, int8_t *min_tx_power)
{
  ...
  status = sl_rail_get_tx_power_config(g_rail_handle, &tx_power_config);
  if (status != SL_RAIL_STATUS_NO_ERROR) {
    SL_SID_LOG_PAL_ERROR("pal rail: sl_rail_supports_tx_power_mode tx_power_mode not supported\n");
    return;
  }

  support_mode = sl_rail_supports_tx_power_mode(g_rail_handle, &tx_power_config.mode, &max_power, &min_power);
  if (!support_mode) {
    SL_SID_LOG_PAL_ERROR("pal rail: sl_rail_supports_tx_power_mode tx_power_mode not supported\n");
    return;
  }
  ...
  min_power_ddbm = sl_rail_util_pa_convert_raw_to_dbm(g_rail_handle, tx_power_config.mode, min_power);
  max_power_ddbm = sl_rail_util_pa_convert_raw_to_dbm(g_rail_handle, tx_power_config.mode, max_power);
...
int32_t efr32xgxx_set_txpower(int8_t power)
{
  ...
  if (!g_tx_pwr_cfg_init_once) {
    status = sl_rail_config_tx_power(g_rail_handle, &drv_ctx->config->tx_power_cfg);
    if (status != SL_RAIL_STATUS_NO_ERROR) {
