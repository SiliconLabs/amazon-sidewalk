...
static sid_error_t bus_serial_spi_xfer(const struct sid_pal_serial_bus_iface *iface,
                                       ...
                                       size_t xfer_size)
{
  struct serial_bus_efr32_spi_ctx *bus = NULL;
  bool success = false;
  ...
  bus = SLI_CONTAINEROF(iface, struct serial_bus_efr32_spi_ctx, iface);

  success = client->client_selector_cb ? client->client_selector_cb(client,
                                                                    SID_PAL_SERIAL_BUS_CLIENT_SELECT,
                                                                    client->client_selector_context)
            : sid_pal_gpio_write(client->client_selector, 0) == SID_ERROR_NONE;
  if (!success) {
  ...
  success = client->client_selector_cb ? client->client_selector_cb(client,
                                                                    ...
                                                                    client->client_selector_context)
            : sid_pal_gpio_write(client->client_selector, 1) == SID_ERROR_NONE;

	return success ? SID_ERROR_NONE : SID_ERROR_GENERIC;
}

static sid_error_t bus_serial_spi_destroy(const struct sid_pal_serial_bus_iface *iface)
{
  return (!iface) ? SID_ERROR_INVALID_ARGS : SID_ERROR_NONE;
}
// -----------------------------------------------------------------------------
//                          Public Function Definitions
// -----------------------------------------------------------------------------
sid_error_t sid_pal_serial_bus_efr32_spi_create(const struct sid_pal_serial_bus_iface **iface,
                                                const void *cfg)
{
  static struct serial_bus_efr32_spi_ctx bus = { 0 };
  const struct sid_pal_serial_bus_efr32_spi_config *config = NULL;

  if (!iface || !cfg) {
    return SID_ERROR_INVALID_ARGS;
  }

  config = (const struct sid_pal_serial_bus_efr32_spi_config *)cfg;

  if (!config->skip_nss_output_set) {
  ...
  sid_pal_gpio_output_mode(SL_PIN_ANTSW, SID_PAL_GPIO_OUTPUT_PUSH_PULL);
  sid_pal_gpio_write(SL_PIN_ANTSW, 1);
#endif
