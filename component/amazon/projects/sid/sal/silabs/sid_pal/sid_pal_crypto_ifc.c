...
//                                Static Variables
// -----------------------------------------------------------------------------
static bool hal_init_done = false;
...
// layer anyway.
static uint8_t ed25519_public_key[32] = { 0 };
static uint8_t ed25519_private_key[64] = { 0 };
...
{
  if (psa_generate_random(rand, size) != PSA_SUCCESS) {
    return SID_ERROR_GENERIC;
  }
...
static sid_error_t efr32_crypto_aes_crypt_cipher( sid_pal_aes_params_t *params,
                                                  const psa_algorithm_t *aes_algo,
                                                  const psa_key_handle_t *key_id)
{
  psa_status_t ret;
  psa_cipher_operation_t cipher_op;

  if (params == NULL || aes_algo == NULL || key_id == NULL) {
    return SID_ERROR_NULL_POINTER;
  }
  ...
  if (psa_cipher_set_iv(&cipher_op, params->iv, params->iv_size) != PSA_SUCCESS) {
    return SID_ERROR_GENERIC;
  }

  if (psa_cipher_update(&cipher_op,
                        params->in,
                        params->in_size,
                        params->out,
                        params->out_size,
                        &(params->out_size)) != PSA_SUCCESS) {
    return SID_ERROR_GENERIC;
  }
  ...
  if (psa_cipher_finish(&cipher_op,
                        params->out,
                        params->out_size,
                        &(params->out_size)) != PSA_SUCCESS) {
    return SID_ERROR_GENERIC;
...
static sid_error_t efr32_crypto_aes_crypt_mac(sid_pal_aes_params_t *params,
                                              const psa_algorithm_t *aes_algo,
                                              const psa_key_handle_t *key_id)
{
  psa_status_t ret;
  ...
  mac_op = psa_mac_operation_init();
  if (psa_mac_sign_setup(&mac_op, *key_id, *aes_algo) != PSA_SUCCESS) {
    return SID_ERROR_GENERIC;
  }

  if (psa_mac_update(&mac_op, params->in, params->in_size) != PSA_SUCCESS) {
    return SID_ERROR_GENERIC;
  }
...
static sid_error_t efr32_crypto_aes_crypt(sid_pal_aes_params_t *params)
{
  ...
  switch (params->mode) {
    ...
    case SID_PAL_CRYPTO_DECRYPT: {
      ...
      sid_ret = efr32_crypto_aes_crypt_cipher(params, &aes_algo, &key_id);
    ...
    case SID_PAL_CRYPTO_MAC_CALCULATE: {
      ...
      sid_ret = efr32_crypto_aes_crypt_mac(params, &aes_algo, &key_id);
      if(sid_ret != SID_ERROR_NONE) {
        psa_destroy_key(key_id);
        return sid_ret;
      }
...
static sid_error_t efr32_crypto_ecc_dsa_set_key(psa_key_attributes_t *key_attr,
                                                const sid_pal_dsa_params_t *params,
                                                psa_algorithm_t *alg)
{
  sid_error_t ret = SID_ERROR_NONE;

  if (params == NULL || key_attr == NULL || alg == NULL) {
    return SID_ERROR_NULL_POINTER;
  }
...
static sid_error_t efr32_crypto_ecc_dsa(sid_pal_dsa_params_t *params)
{
  ...
  if (params->algo == SID_PAL_EDDSA_ED25519) {
    ...
    if (params->mode == SID_PAL_CRYPTO_SIGN) {
      if (ed25519_keys_present == false) {
        if (ed25519_CreateKeyPair(ed25519_public_key,
                                  ed25519_private_key,
                                  NULL,
                                  params->key) != 0) {
          return SID_ERROR_GENERIC;
      ...
      if (ed25519_SignMessage(params->signature,
                              ed25519_private_key,
                              NULL,
                              params->in,
                              params->in_size) == 0) {
      ...
      if (ed25519_VerifySignature(params->signature,
                                  params->key,
                                  params->in,
                                  params->in_size) == 1) {
        return SID_ERROR_NONE;
  ...
  sid_ret = efr32_crypto_ecc_dsa_set_key(&key_attr, params, &alg);
  if (sid_ret != SID_ERROR_NONE) {
    return sid_ret;
  }
  ...
  switch (params->mode) {
    ...
    case SID_PAL_CRYPTO_VERIFY:
      ...
      if (psa_verify_message(key_id,
                             alg,
                             params->in,
                             params->in_size,
                             params->signature,
                             params->sig_size) != PSA_SUCCESS) {
        psa_destroy_key(key_id);
        return SID_ERROR_GENERIC;
      }
      break;
  ...
  if ((params->mode == SID_PAL_CRYPTO_VERIFY
       && secure_vault_enabled)
       || !secure_vault_enabled) {
    ret = psa_destroy_key(key_id);
  }
...
static sid_error_t efr32_crypto_ecc_key_gen(sid_pal_ecc_key_gen_params_t *params)
{
  ...
  if (params->algo == SID_PAL_EDDSA_ED25519) {
    ...
    if (ed25519_keys_present == false) {
      if (efr32_crypto_rand(random, 32) != SID_ERROR_NONE) {
        return SID_ERROR_GENERIC;
      }
  ...
  if (secure_vault_enabled
      && (params->algo == SID_PAL_EDDSA_ED25519 || params->algo == SID_PAL_ECDSA_SECP256R1)) {
