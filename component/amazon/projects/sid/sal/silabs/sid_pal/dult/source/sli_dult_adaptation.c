                              ...
                              void *data)
{
  ...
  sl_status_t sc;
  queue_item_t item;
  sli_dult_timer_t *timer = (sli_dult_timer_t *) data;
                                         ...
                                         uint16_t data_size)
{
  ...
  queue_item_t item = {.type          = QUEUE_ITEM_TYPE_DISPATCH,
                       .data.dispatch = {.task = task, .data_size = data_size}};
...
{
  bool running;
  sl_status_t sc = sl_sleeptimer_is_timer_running(&timer->handle, &running);
  SLI_DULT_ASSERT_ST(sc);
  return running;
}
