                                                 ...
                                                 uint16_t len)
{
  ...
  if (status != SL_STATUS_OK) {
    DULT_LOG_ERROR("%s: Failed to get dult context: %#x", __func__, (unsigned)status);
    return status;
  }
...
static sl_status_t get_product_data_handler(sli_dult_ctx_t *dult_ctx)
{
  ...
  status = sl_bt_gatt_server_send_indication(dult_ctx->connection,
                                             ...
                                             (const uint8_t *) &response);
  if (status) {
    DULT_LOG_ERROR("%s: Failed to send indication: %#x", __func__, (unsigned)status);
    return SL_STATUS_FAIL;
  }
  return status;
}
...
sl_status_t sli_dult_non_user_add_service(void)
{
  ...
  if (sc != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_gattdb_new_session error = %#x", (unsigned)sc);
  }
  ...
  if (sc != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_gattdb_add_service error = %#x", (unsigned)sc);
  }
  ...
  if (sc != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_gattdb_add_uuid128_characteristic error = %#x", (unsigned)sc);
  ...
  if (sc != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_gattdb_start_service error = %#x", (unsigned)sc);
  }
  ...
  if (sc != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_gattdb_start_characteristic error = %#x", (unsigned)sc);
  }

  // Save changes and close the database editing session
  sl_bt_gattdb_commit(dult_non_user_session);
  if (sc != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_gattdb_commit error = %#x", (unsigned)sc);
  }
  return sc;
}
#endif
