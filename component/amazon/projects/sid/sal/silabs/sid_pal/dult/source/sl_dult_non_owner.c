...
static sl_status_t
    get_protocol_implementation_version_handler(sli_dult_ctx_t *dult_ctx);
    ...
    get_protocol_implementation_version_handler(sli_dult_ctx_t *dult_ctx)
{
  ...
  size_t len;
  response.data.protocol_implementation_version =
      dult_ctx->info.protocol_implementation_version;
  len = sizeof(sl_dult_opcode_t) +
        sizeof(sl_dult_protocol_implementation_version_t);
...
sl_status_t sli_dult_non_user_add_service(void)
{
  ...
  sc = sl_bt_gattdb_add_uuid128_characteristic(
      dult_non_user_session, // Database update session
