...
static sl_status_t sound_state_change_notify(sli_dult_ctx_t *dult_ctx)
{
  ...
  if (dult_ctx->sound.active == false) {
    ...
    if (dult_ctx->sound.state == SLI_DULT_SOUND_STATE_START_REQUEST &&
      ...
      sc = sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_START, SL_STATUS_OK);
      if (sc != SL_STATUS_OK) {
    ...
    if (dult_ctx->sound.state == SLI_DULT_SOUND_STATE_START_REQUEST &&
      ...
      sc = sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_START, SL_STATUS_INVALID_STATE);
      if (sc != SL_STATUS_OK) {
...
sl_status_t sli_dult_bt_sound_start(sli_dult_ctx_t *dult_ctx)
{
  ...
  if (dult_ctx->near_owner_state != SL_DULT_STATE_SEPARATED) {
    return sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_START, SL_STATUS_FAIL);
  }

  if (!dult_ctx->sound.is_initialized) {
    return sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_START, SL_STATUS_NOT_INITIALIZED);
  }
  ...
  if (dult_ctx->sound.state != SLI_DULT_SOUND_STATE_IDLE) {
    return sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_START, SL_STATUS_INVALID_STATE);
  }
...
sl_status_t sli_dult_bt_sound_stop(sli_dult_ctx_t *dult_ctx)
{
  ...
  if (!dult_ctx->sound.is_initialized) {
    return sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_STOP, SL_STATUS_NOT_INITIALIZED);
  }

  /*The sound maker MUST play sound for a minimum duration of 5 seconds*/
  if (current_time - dult_ctx->sound.start_time <
      DULT_PLAY_SOUND_DURATION_MIN_MS) {
    return sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_STOP, SL_STATUS_INVALID_STATE);
  }
  /* Sound stop only available when the accessory is in the separated state*/
  if (dult_ctx->near_owner_state != SL_DULT_STATE_SEPARATED) {
    return sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_STOP, SL_STATUS_FAIL);
  }
  if (dult_ctx->sound.state != SLI_DULT_SOUND_STATE_START_ACK) {
    return sli_dult_sound_cmd_resp(dult_ctx, SLI_DULT_OC_NON_OWNER_SOUND_STOP, SL_STATUS_INVALID_STATE);
  }
