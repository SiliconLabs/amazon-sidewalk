...
sl_status_t sl_dult_update_motion_detection(uint8_t handle, bool status)
{
  ...
  if (sc != SL_STATUS_OK) {
    DULT_LOG_ERROR("%s: Invalid dult handle: %#x", __func__, (unsigned)sc);
    return sc;
  }
                                            ...
                                            bool started)
{
  ...
  sl_status_t sc;
  sli_dult_timer_t *poll_duration_timer = &dult_ctx->motion_ctx.timer.motion_poll_duration;
...
static void backoff_setup(sli_dult_ctx_t *dult_ctx)
{
  ...
  state_reset(dult_ctx);
  sc = sli_dult_adaptation_timer_start(&dult_ctx->motion_ctx.timer.motion_enable,
...
{
  sl_status_t sc = SL_STATUS_OK;
  uint16_t separated_timeout_seed = 0;
  size_t separated_timeout_seed_len = 0;
  uint32_t separated_timeout_minute = 0;
  ...
  DULT_LOG_DEBUG("Waiting for separated timeout: %u minutes",
                 (unsigned)separated_timeout_minute);
}
...
{
  sli_dult_ctx_t *dult_ctx = (sli_dult_ctx_t *) ctx;
  sl_dult_motion_detection_event_action_t evt_action = SL_DULT_MOTION_DETECTION_EVENT_TYPE_GET_MOTION_STATUS;
