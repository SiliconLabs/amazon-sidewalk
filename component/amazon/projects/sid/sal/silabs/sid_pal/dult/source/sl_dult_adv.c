...
sl_status_t sli_dult_adv_int_payload(sli_dult_ctx_t *dult_ctx, bool rotation)
{
  ...
  if (status != SL_STATUS_OK) {
    DULT_LOG_ERROR("Failed to set advertisement data: %#lx", status);
  }
  return status;
}
...
sl_status_t sl_dult_start(uint8_t handle)
{
  ...
  if (dult_ctx->info.adv_handle == NULL) {
    if (dult_ctx->adv.handle == SLI_DULT_ADV_SET_INVALID) {
      ...
      if (status != SL_STATUS_OK) {
        DULT_LOG_ERROR("Failed to create advertisement set: %#lx", status);
        return status;
      }
    }
  }
  ...
  if (status != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_advertiser_set_timing failed: %#lx", status);
    return status;
  }
  ...
  if (status != SL_STATUS_OK) {
    DULT_LOG_ERROR("sl_bt_legacy_advertiser_start failed: %#lx", status);
    return status;
  }
...
static void scheduler_mac_address_rotation_timer(sli_dult_ctx_t *dult_ctx)
{
  ...
  DULT_LOG_DEBUG("Rotation timer updated to %ld ms", interval);
}
