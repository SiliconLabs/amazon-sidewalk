...
sl_status_t sl_dult_init(const sl_dult_accessory_info_t *info, uint8_t *handle)
{
  ...
  if (status != SL_STATUS_OK) {
    DULT_LOG_ERROR("sli_dult_register failed: %#x", (unsigned)status);
    return status;
  }
...
sl_status_t sl_dult_deinit(uint8_t handle)
{
  ...
  if (dult_ctx->adv.started) {
    ...
    if (status != SL_STATUS_OK) {
      DULT_LOG_ERROR("Failed to stop advertisement: %#x", (unsigned)status);
    }
  }
  ...
  if (dult_ctx->info.adv_handle == NULL) {
    if (dult_ctx->adv.handle != SLI_DULT_ADV_SET_INVALID) {
      ...
      if (status != SL_STATUS_OK) {
        DULT_LOG_ERROR("Failed to delete advertisement set: %#x", (unsigned)status);
      }
    }
  }
  ...
  if (status != SL_STATUS_OK) {
    DULT_LOG_ERROR("sli_dult_unregister failed: %#x", (unsigned)status);
  }
  return status;
}
...
void sli_bt_dult_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_gatt_server_user_write_request_id:
      ...
      if (sc == SL_STATUS_OK) {
        if (sli_dult_get_non_owner_characteristic_handle() ==
          ...
          if (sc != SL_STATUS_OK) {
            DULT_LOG_ERROR("Non-owner operation handler failed: %#x", (unsigned)sc);
          }
        }
      }
      break;
    ...
    case sl_bt_evt_system_stopped_id:
      ...
      if (sc != SL_STATUS_OK) {
        DULT_LOG_ERROR("Failed to handle ble system stopped: %#x", (unsigned)sc);
      }
      break;
