 ...
 */
/**
 * \addtogroup sid_pal_radio_subghz_ifc
 * @{
 */
...
/**************************************************************************//**
 * @addtogroup sid_pal_radio_subghz_ifc_types Type definitions
 * @ingroup sid_pal_radio_subghz_ifc
 * @{
 *****************************************************************************/

#define SID_PAL_RADIO_RX_PAYLOAD_MAX_SIZE 255  /*!< Defines the maximum size of the payload that can be received by the radio. */

/**
 * @brief Radio Error Codes
 */
#define    RADIO_ERROR_NONE                0   /*!< No error */
#define    RADIO_ERROR_NOT_SUPPORTED       -1  /*!< Feature not supported */
#define    RADIO_ERROR_INVALID_PARAMS      -2  /*!< parameters not valid */
#define    RADIO_ERROR_IO_ERROR            -3  /*!< GPIO error on DIO pins */
...
#define    RADIO_ERROR_PKT_CHECK_DCR_LIMIT -9  /*!< DCR (Duty Cycle Restriction) check fail */
...
#define    RADIO_ERROR_VENDOR_LAST         -255 /*!< Add vendor error codes between -64 and -255 */

/**
 * @brief Sidewalk Phy Radio State
 */
#define    SID_PAL_RADIO_UNKNOWN              0 /*!< Unknown radio state. */
#define    SID_PAL_RADIO_STANDBY              1 /*!< Standby state of the radio. */
#define    SID_PAL_RADIO_SLEEP                2 /*!< Sleep state of the radio. */
#define    SID_PAL_RADIO_RX                   3 /*!< Receive state of the radio. */
#define    SID_PAL_RADIO_TX                   4 /*!< Transmit state of the radio. */
#define    SID_PAL_RADIO_CAD                  5 /*!< Channel Activity Detection (CAD) state of the radio. */
#define    SID_PAL_RADIO_STANDBY_XOSC         6 /*!< Standby state with an external oscillator (XOSC) of the radio. */
#define    SID_PAL_RADIO_RX_DC                7 /*!< Receive duty cycle state of the radio. */
#define    SID_PAL_RADIO_BUSY                 8 /*!< Busy state of the radio. */

/**
 * @brief Sidewalk Phy Radio Region For Regional Config
 */
typedef enum {
...
/**
 * @brief Sidewalk Phy Radio Modem Mode
 */
typedef enum sid_pal_radio_modem_mode {
    SID_PAL_RADIO_MODEM_MODE_FSK      = 0,  /*!< Frequency Shift Keying (FSK) modem mode */
    SID_PAL_RADIO_MODEM_MODE_LORA     = 1,  /*!< Long Range (LoRa) modem mode */
} sid_pal_radio_modem_mode_t;

/**
 * @brief Sidewalk Phy Radio Event
 */
typedef enum {
    SID_PAL_RADIO_EVENT_UNKNOWN       = 0,  /*!< Unknown event */
    SID_PAL_RADIO_EVENT_TX_DONE       = 1,  /*!< Transmission done event */
    SID_PAL_RADIO_EVENT_RX_DONE       = 2,  /*!< Reception done event */
    SID_PAL_RADIO_EVENT_CAD_DONE      = 3,  /*!< Channel Activity Detection (CAD) done event */
    SID_PAL_RADIO_EVENT_CAD_TIMEOUT   = 4,  /*!< CAD timeout event */
    SID_PAL_RADIO_EVENT_RX_ERROR      = 5,  /*!< Reception error event */
    SID_PAL_RADIO_EVENT_TX_TIMEOUT    = 6,  /*!< Transmission timeout event */
    SID_PAL_RADIO_EVENT_RX_TIMEOUT    = 7,  /*!< Reception timeout event */
    SID_PAL_RADIO_EVENT_CS_DONE       = 8,  /*!< Carrier Sense (CS) done event */
    SID_PAL_RADIO_EVENT_CS_TIMEOUT    = 9,  /*!< CS timeout event */
    SID_PAL_RADIO_EVENT_HEADER_ERROR  = 10, /*!< Header error event */
    SID_PAL_RADIO_EVENT_SYNC_DET      = 11, /*!< Sync detection event */
} sid_pal_radio_events_t;

/**
 * @brief Sidewalk Phy Radio Data Rate
 */
typedef enum sid_pal_radio_data_rate {
    SID_PAL_RADIO_DATA_RATE_INVALID   = 0,  /*!< Invalid data rate */
    SID_PAL_RADIO_DATA_RATE_2KBPS     = 1,  /*!< 2 Kbps data rate */
    SID_PAL_RADIO_DATA_RATE_22KBPS    = 2,  /*!< 22 Kbps data rate */
    SID_PAL_RADIO_DATA_RATE_50KBPS    = 3,  /*!< 50 Kbps data rate */
    SID_PAL_RADIO_DATA_RATE_150KBPS   = 4,  /*!< 150 Kbps data rate */
    SID_PAL_RADIO_DATA_RATE_250KBPS   = 5,  /*!< 250 Kbps data rate */
    SID_PAL_RADIO_DATA_RATE_12_5KBPS  = 6,  /*!< 12.5 Kbps data rate */
    SID_PAL_RADIO_DATA_RATE_CUSTOM   = 7,   /*!< Custom data rate */
    SID_PAL_RADIO_DATA_RATE_MAX_NUM   = SID_PAL_RADIO_DATA_RATE_CUSTOM, /*!< 0 is not a valid data rate */
} sid_pal_radio_data_rate_t;

/**
 * @brief Sidewalk Phy Radio CAD (Channel Activity Detection) Exit Mode
 */
typedef enum {
...
/**
 * @brief Radio IRQ MASK
 */
typedef enum {
    RADIO_IRQ_NONE                 = (0 << 0),              /*!< No interrupt */
    RADIO_IRQ_TX_DONE              = (1 << 0),              /*!< Transmission done interrupt */
    RADIO_IRQ_RX_DONE              = (1 << 1),              /*!< Reception done interrupt */
    RADIO_IRQ_PREAMBLE_DETECT      = (1 << 2),              /*!< Preamble detection interrupt */
    RADIO_IRQ_VALID_SYNC_WORD      = (1 << 3),              /*!< Valid sync word interrupt */
    RADIO_IRQ_VALID_HEADER         = (1 << 4),              /*!< Valid header interrupt */
    RADIO_IRQ_ERROR_HEADER         = (1 << 5),              /*!< Header error interrupt */
    RADIO_IRQ_ERROR_CRC            = (1 << 6),              /*!< CRC error interrupt */
    RADIO_IRQ_CAD_DONE             = (1 << 7),              /*!< Channel Activity Detection (CAD) done interrupt */
    RADIO_IRQ_CAD_DETECT           = (1 << 8),              /*!< CAD detection interrupt */
    RADIO_IRQ_TXRX_TIMEOUT         = (1 << 9),              /*!< Transmission or reception timeout interrupt */
    RADIO_IRQ_ALL                  = ((1 << 10 ) - 1),      /*!< All interrupts */
...
typedef struct sid_pal_radio_rx_packet {
    sid_pal_radio_data_rate_t data_rate;                            /*!< Data rate at which the packet was received */
    sid_pal_radio_lora_rx_packet_status_t lora_rx_packet_status;    /*!< LoRa-specific packet status */
    sid_pal_radio_fsk_rx_packet_status_t fsk_rx_packet_status;      /*!< FSK-specific packet status */
    struct sid_timespec rcv_tm;                                     /*!< Timestamp when the packet was received */
    uint8_t rcv_payload[SID_PAL_RADIO_RX_PAYLOAD_MAX_SIZE];         /*!< Buffer to store the received payload */
    uint8_t payload_len;                                            /*!< Length of the received payload */
} sid_pal_radio_rx_packet_t;

/**
 * @brief Sidewalk Phy settings to configure radio prior to transmission and reception
 */
typedef struct sid_pal_radio_packet_cfg {
    sid_pal_radio_data_rate_t data_rate;    /*!< Data rate for the packet */
    uint8_t channel;                        /*!< Channel on which the packet will be transmitted or received */
    uint8_t invert_IQ;                      /*!< Flag to indicate if IQ inversion is enabled */
    uint16_t preamble_len;                  /*!< Length of the preamble */
    uint32_t symbol_timeout;                /*!< Symbol timeout value */
    bool crc_enabled;                       /*!< Flag to indicate if CRC is enabled */
} sid_pal_radio_packet_cfg_t;

/**
 * @brief Sidewalk Phy transmit packet configuration
 */
typedef struct sid_pal_radio_tx_packet {
    sid_pal_radio_packet_cfg_t packet_cfg;  /*!< Configuration for the packet */
    int8_t tx_power_in_dbm;                 /*!< Transmit power in dBm */
    uint8_t *tx_payload;                    /*!< Pointer to the payload to be transmitted */
    uint8_t payload_len;                    /*!< Length of the payload */
    uint32_t timeout;                       /*!< Timeout for the transmission */
} sid_pal_radio_tx_packet_t;

/**
 * @brief Sidewalk radio state transition timings
 */
typedef struct sid_pal_radio_state_transition_timings {
    uint32_t sleep_to_full_power_us;    /*!< Time to transition from sleep to full power state, in microseconds */
    uint32_t full_power_to_sleep_us;    /*!< Time to transition from full power to sleep state, in microseconds */
 ...
 *
 * @param[in] sleep_us time period for which the radio should be put in sleep mode
 *
 * @retval  On success RADIO_ERROR_NONE, on error a negative number is returned
 */
 ...
 *
 * @param[in]   cad_params in microseconds for how long radio is in
 ...
 *
 * @param[out] state_delay of switching delay between different radio states
 *
 * @retval  On success RADIO_ERROR_NONE, on error a negative number is returned
 */
...
/**
 * @brief Calculates the minimum number of symbols that takes more than delay_micro_secs of air time.
 *
 * In the case of a fractional number of symbols, the return value is rounded up to the next integer.
 * Does not affect the radio state and can be executed without radio ownership.
 * In the case of an error, 0 is returned.
 *
 * @param[in]   mod_params Current modulation parameters that phy uses. If null, zero will be returned.
 * @param[in]   delay_micro_secs Input amount of time in uS that will be translated to number of symbols.
 *
 * @retval  number of symbols
 *
 */
 ...
 *
 * @param[inout]   tx_pkt_cfg tx packet config
 *
 * @retval  On success RADIO_ERROR_NONE, on error a negative number is returned
 */
...
/** @} */ // end of sid_pal_radio_subghz_ifc group