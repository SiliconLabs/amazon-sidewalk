 ...
 */
/**
 * \addtogroup sid_pal_sclient_ifc
 * @{
 */
...
/**************************************************************************//**
 * @addtogroup sid_pal_sclient_ifc_types Type definitions
 * @ingroup sid_pal_sclient_ifc
 * @{
 *****************************************************************************/
...
typedef struct {
    ...
    /**
     * @brief Declaration of function type. The function itself is callback which can be called by
    ...
    /**
     * @brief Declaration of pointer to function. The function itself is callback which can be called by
    ...
    /**
     * @brief Notifies that some data has been received and can be fetched by method @ref sid_pal_serial_ifc_s::get_frame
     *
...
/* TODO: remove "params_ctx" as soon as all implementations of serial client interface
   are reworked and HTP (Host Transport Protocol) has removed from there. */
    ...
    const struct sid_event_queue *queue;           /*!< Pointer to the event queue structure. */
} sid_pal_serial_params_t;

/**
 * @typedef sid_pal_serial_ifc_t
 * @brief Type definition for a constant pointer to a structure representing the serial client interface.
 */
...
struct sid_pal_serial_ifc_s {
    ...
    /**
     * @brief Pointer to implementation-depended method which send data in "buffer_to_send" with
    ...
    /**
     * @brief Fetches frame if one was received. It is advisable to copy it instantaneously, since there is no
    ...
    /* TODO: Remove "process" method as soon as all implementations of serial client interface
       are reworked and HTP (Host Transport Protocol) has removed from there. */
    /**
     * @brief Pointer to implementation-depended method which executes routine to sustain connection with the host.
     *
     * @param [in] _this            -   pointer to interface which is a part of implementation instance.
     *
    ...
    /**
     * @brief Pointer to implementation-depended method. It retrieves mtu (maximum transmit unit).
     *
     * @param [in] _this        -   pointer to interface which is a part of implementation instance.
     *
    ...
    /**
     * @brief Pointer to implementation-depended method which free memory taken by specific instance.
     *
     * @param [in] _this            -   pointer to interface which is a part of implementation instance.
     */
...
/**
 * @struct sid_pal_serial_client_factory_t
 * @brief Generic type for instance factory.
 */
typedef struct {
    /**
     * @brief General type for constructors of this interface. It's advised to use it.
     *
    ...
    const void * config;        /*!< The configuration to be passed in sid_pal_serial_create. */
} sid_pal_serial_client_factory_t;

/** @} (end sid_pal_sclient_ifc_types) */
...
/** @} */ // end of sid_pal_sclient_ifc group