...
/* Sailboat Application Service Id */
#define SID_MSG_DESTINATION_AWS_IOT_CORE 33
/* Mobile D2D Application Service Id */
#define SID_MSG_DESTINATION_MOBILE_D2D 7
...
static const struct sid_address aws_iot_core = {
...
/*
 * Check if the given raw address is the same as local address
 *
 * @param[in] raw_addr1 Pointer1 to the sidewalk raw address that
 *                      needs to be checked
 *
 * @returns bool true if the address is same or false otherwise
 */
bool sid_address_is_local_raw(const void *raw_addr);
 ...
 * @returns #SID_ERROR_NONE on success
 */
sid_error_t sid_address_set_local(const struct sid_address *const address, bool flush);
...
/*
 * Reset local sid_address
 *
 * This api call flushes the sid_address cache and forces
 * sid network module to create the address from SMSN if not registered or from
 * the kv store if regsitered
 */
void sid_address_reset(void);
