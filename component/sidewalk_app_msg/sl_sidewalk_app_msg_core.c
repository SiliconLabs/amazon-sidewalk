...
static sl_sid_app_msg_st_t serialize(sl_sid_app_msg_t *msg,
                                     uint8_t **pkt,
                                     uint16_t *pkt_len);
static sl_sid_app_msg_st_t deserialize(sl_sid_app_msg_t **msg,
                                       uint8_t *pkt,
                                       uint16_t pkt_len);
static sl_sid_app_msg_st_t validate(sl_sid_app_msg_t *msg);
static sl_sid_app_msg_st_t receive(sl_sid_app_msg_t **rcvd_app_msg,
                                   const struct sid_msg *rcvd_sid_msg);
static sl_sid_app_msg_st_t sli_sid_app_msg_prepare_op_for_send(sl_sid_app_msg_op_t op_rcv,
...
sl_sid_app_msg_st_t sl_sid_app_msg_prepare_sid_msg(sl_sid_app_msg_t *app_msg,
...
sl_sid_app_msg_st_t sli_sid_app_msg_prepare_send(sl_sid_app_msg_t *app_msg,
                                                 uint8_t cmd_cls,
                                                 uint8_t cmd_id,
                                                 sl_sid_app_msg_op_t op,
                                                 uint8_t seq,
                                                 void *val,
                                                 uint16_t val_len)
{
  ...
  if (op_send == SL_SID_APP_MSG_OP_NTFY
      || op_send == SL_SID_APP_MSG_OP_SET
      || op_send == SL_SID_APP_MSG_OP_GET) {
    uint8_t seq_gen;
  ...
  app_msg->length = (uint8_t)val_len;
...
static sl_sid_app_msg_st_t receive(sl_sid_app_msg_t **app_msg,
                                   const struct sid_msg *sid_msg)
{
  SLI_SID_APP_MSG_RETURN_ST_IF_PARAM_INVALID_2(app_msg, sid_msg);

  SLI_SID_APP_MSG_RETURN_ST_IF_FAILED(deserialize(app_msg,
                                      (uint8_t *)sid_msg->data,
                                      (uint16_t)sid_msg->size));
  SLI_SID_APP_MSG_RETURN_ST_IF_FAILED(validate(*app_msg));
