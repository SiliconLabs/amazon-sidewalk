...
sl_sid_app_msg_st_t sl_sid_app_msg_sid_mtu_prepare_send(sl_sid_app_msg_sid_mtu_ctx_t *ctx,
                                                        sl_sid_app_msg_t *send_app_msg)
{
  SLI_SID_APP_MSG_CLR_PROCESSING_FLAG(ctx);

  if (ctx->hdl.operation == SL_SID_APP_MSG_OP_GET
      || ctx->hdl.operation == SL_SID_APP_MSG_OP_NTFY) {
    return SLI_SID_APP_MSG_PREP_SEND_PARAM_FUNC(SLI_SID_APP_MSG_CMD_CLS_SID,
                                                SLI_SID_APP_MSG_CMD_ID_SID_MTU);
  } else {
    return SL_SID_APP_MSG_ERR_ST_APP_WRONG_OP;
  }
}

sl_sid_app_msg_st_t sl_sid_app_msg_sid_time_prepare_send(sl_sid_app_msg_sid_time_ctx_t *ctx,
                                                         sl_sid_app_msg_t *send_app_msg)
{
  SLI_SID_APP_MSG_CLR_PROCESSING_FLAG(ctx);
...
SL_WEAK void sl_sid_app_msg_sid_mtu_cb(sl_sid_app_msg_sid_mtu_ctx_t *ctx)
{
  (void)ctx;
}

SL_WEAK void sl_sid_app_msg_sid_time_cb(sl_sid_app_msg_sid_time_ctx_t *ctx)
{
  (void)ctx;
}
