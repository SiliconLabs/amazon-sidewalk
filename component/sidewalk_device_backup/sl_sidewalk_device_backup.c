...
void sl_sidewalk_device_backup_handle_backup_restore(void)
{
  ...
  SID_PAL_LOG_INFO("backup/restore: %s ok", __func__);
}
...
static void erase_user_data(void)
{
  ...
  app_assert(st == SL_STATUS_OK, "backup/restore: msc clock enable failed, st: %ld", st);
  MSC_Status_TypeDef ret = MSC_ErasePage((uint32_t *)USERDATA_BASE);
  app_assert(ret == mscReturnOk, "backup/restore: @userdata erase failed, ret: %d", ret);
#else
  sl_status_t ret = sl_se_erase_user_data(&cmd_ctx);
  app_assert(ret == SL_STATUS_OK, "backup/restore: @userdata erase failed, ret: %ld", ret);
#endif
}
...
static void write_user_data(uint32_t offset, void *data, uint32_t data_len)
{
  app_assert(data_len % sizeof(uint32_t) == 0, "backup/restore: @userdata write failed due to unaligned data, off: %.3ld, len: %.3ld", offset, data_len);
#if (defined(EFR32XG24) || defined(EFR32XG28))
  MSC_Init();
  MSC_Status_TypeDef ret = MSC_WriteWord(((uint32_t *)USERDATA_BASE + offset), data, data_len);
  app_assert(ret == mscReturnOk, "backup/restore: @userdata write failed, off: %.3ld, len: %.3ld, ret: %d", offset, data_len, ret);
  MSC_Deinit();
#else
  sl_status_t ret = sl_se_write_user_data(&cmd_ctx, (sizeof(uint32_t) * offset), data, data_len);
  app_assert(ret == SL_STATUS_OK, "backup/restore: @userdata write failed, off: %.3ld, len: %.3ld, ret: %ld", offset, data_len, ret);
#endif
}
...
static void read_user_data(uint32_t offset, uint8_t *data, uint32_t data_len)
{
  app_assert(data_len % sizeof(uint32_t) == 0, "backup/restore: @userdata read failed due to unaligned data, off: %.3ld, len: %.3ld", offset, data_len);
...
static bool are_wrapped_keys_present(void)
{
  ...
  if (!crpyto_obj_keys) {
    SID_PAL_LOG_ERROR("backup/restore: %s memory allocation failed", __func__);
    return false;
  }
...
static bool is_backup_needed(void)
{
  ...
  if (needed) {
    SID_PAL_LOG_INFO("backup/restore: backup needed");
  } else {
    SID_PAL_LOG_INFO("backup/restore: backup not needed");
  }
  return needed;
}
...
{
#if defined(SV_ENABLED)
  bool needed = (!is_apid_valid() || !are_wrapped_keys_present());
#else
  bool needed = !is_apid_valid();
#endif // SV_ENABLED
  if (needed) {
    SID_PAL_LOG_INFO("backup/restore: restore needed");
  } else {
    SID_PAL_LOG_INFO("backup/restore: restore not needed");
  }
  return needed;
}
...
static bool is_backup_possible(void)
{
  bool ret = false;
#if defined(SV_ENABLED)
  ret = are_wrapped_keys_present();
  app_assert(ret == true, "backup/restore: device not provisioned yet");
#endif // SV_ENABLED
  ret = is_apid_valid();
  app_assert(ret == true, "backup/restore: static cert not matching with mfg info");
  SID_PAL_LOG_INFO("backup/restore: backup possible");
  return true;
}
...
static bool is_restore_possible(void)
{
  ...
  if (possible) {
    SID_PAL_LOG_INFO("backup/restore: restore possible");
  }
  app_assert(possible == true, "backup/restore: restore not possible");
  return possible;
}
...
static void perform_backup(void)
{
  SID_PAL_LOG_INFO("backup/restore: backup started");
  ...
  erase_user_data();
  SID_PAL_LOG_INFO("backup/restore: @userdata erased");
  ...
  for (uint8_t i = 0; i < sizeof(MFG_OBJECT_TABLE) / sizeof(mfg_obj_t); i++) {
    if (MFG_OBJECT_TABLE[i].val == NULL) {
      ...
      SID_PAL_LOG_INFO("backup/restore: mfg obj written to @userdata, key: 0x%.5lx, off: %.3ld, len: %.3ld", MFG_OBJECT_TABLE[i].key, offset, len);
  ...
  for (uint32_t key = ITS_OBJ_RANGE_START; key < ITS_OBJ_RANGE_END; key++) {
    ...
    if (st == ECODE_NVM3_OK && obj_type == NVM3_OBJECTTYPE_DATA && obj_len == WRAPPED_KEY_LEN) {
      ...
      app_assert(st == ECODE_NVM3_OK, "backup/restore: default nvm3 object cannot be read, key: 0x%.5lx", key);
      uint32_t len = 0;
      ...
      write_user_data(offset, read_buffer, len);
      SID_PAL_LOG_INFO("backup/restore: wrp key written to @userdata, key: 0x%.5lx, off: %.3ld, len: %.3ld", key, offset, len);
      offset += len / sizeof(uint32_t);
    }
  }
#endif // SV_ENABLED
  ...
  SID_PAL_LOG_INFO("backup/restore: backup completed with %ld bytes written to @userdata", (offset * sizeof(uint32_t)));
}
...
static void perform_restore(void)
{
  SID_PAL_LOG_INFO("backup/restore: restore started");
  ...
  app_assert(st == ECODE_NVM3_OK, "backup/restore: default nvm3 inst cannot be erased, st: %ld", st);
  SID_PAL_LOG_INFO("backup/restore: default nvm3 inst erased");
  ...
  for (uint8_t i = 0; i < sizeof(MFG_OBJECT_TABLE) / sizeof(mfg_obj_t); i++) {
    if (MFG_OBJECT_TABLE[i].val == NULL) {
      ...
      SID_PAL_LOG_INFO("backup/restore: dyn obj written to default nvm3 inst, key: 0x%.5lx, len: %.3ld", MFG_OBJECT_TABLE[i].key, MFG_OBJECT_TABLE[i].len);
      offset += len / sizeof(uint32_t);
    } else {
      // common data
      ret = sid_pal_mfg_store_write((int)MFG_OBJECT_TABLE[i].key, MFG_OBJECT_TABLE[i].val, MFG_OBJECT_TABLE[i].len);
      SID_PAL_LOG_INFO("backup/restore: sta obj written to default nvm3 inst, key: 0x%.5lx, len: %.3ld", MFG_OBJECT_TABLE[i].key, MFG_OBJECT_TABLE[i].len);
      bytes_written_sta_obj += MFG_OBJECT_TABLE[i].len;
    }
    app_assert(ret == 0, "backup/restore: obj cannot be written, key: 0x%.5lx, ret: %ld", MFG_OBJECT_TABLE[i].key, ret);
  }
  ...
  for (uint8_t i = 0; i < WRAPPED_KEY_CNT; i++) {
    ...
    app_assert(st == ECODE_NVM3_OK, "backup/restore: obj cannot be written, key: 0x%.5lx, len: %.3d, st: %ld", key, WRAPPED_KEY_LEN, st);
    if (nvm3_repackNeeded(nvm3_defaultHandle)) {
      st = nvm3_repack(nvm3_defaultHandle);
      app_assert(st == ECODE_NVM3_OK, "backup/restore: default nvm3 inst repack failed, key: 0x%.5lx, len: %.3d, st: %ld", key, WRAPPED_KEY_LEN, st);
    }
    SID_PAL_LOG_INFO("backup/restore: wrp key written to default nvm3 inst, key: 0x%.5lx, len: %.3d", key, WRAPPED_KEY_LEN);
    offset += WRAPPED_KEY_LEN / sizeof(uint32_t);
  }
#endif // SV_ENABLED

  SID_PAL_LOG_INFO("backup/restore: restore completed with %ld bytes written to default nvm3 inst", (offset * sizeof(uint32_t)) + bytes_written_sta_obj);
}
