...
static bool is_backup_needed(void)
{
  bool needed = !is_magic_number_present();
  if (needed) {
...
static bool is_restore_possible(void)
{
  bool possible = is_magic_number_present();
  if (possible) {
...
static void perform_backup(void)
{
  ...
  for (uint8_t i = 0; i < sizeof(MFG_OBJECT_TABLE) / sizeof(mfg_obj_t); i++) {
    if (MFG_OBJECT_TABLE[i].val == NULL) {
      ...
      uint32_t len = 0;
      if (MFG_OBJECT_TABLE[i].len % sizeof(uint32_t) == 0) {
        // aligned data
        len = (uint32_t)MFG_OBJECT_TABLE[i].len;
      } else {
...
static void perform_restore(void)
{
  ...
  for (uint8_t i = 0; i < sizeof(MFG_OBJECT_TABLE) / sizeof(mfg_obj_t); i++) {
    ...
    app_assert(ret == 0, "pal backup: obj cannot be written, key: 0x%.5lx, ret: %ld", MFG_OBJECT_TABLE[i].key, ret);
  }
