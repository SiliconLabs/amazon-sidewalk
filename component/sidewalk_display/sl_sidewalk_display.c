...
static void draw_qr(display_t *disp_hnd, sli_sidewalk_qr_code_qr_t *qr_buffer);
static void handle_status_msg(display_t *disp_hnd, const sl_sidewalk_display_msg_t *message);
static void handle_qr_msg(display_t *disp_hnd, sl_sidewalk_display_msg_t *message);
static void handle_normal_msg(display_t *disp_hnd, const sl_sidewalk_display_msg_t *message);
...
static display_t display_handler = { 0 };
...
bool sl_sidewalk_display_update(void)
{
...
static void draw_qr(display_t *disp_hnd, sli_sidewalk_qr_code_qr_t *qr_buffer)
{
  ...
  GLIB_clear(&disp_hnd->glibContext);
  ...
  for (uint8_t x = 0; x < qr_buffer->width; x++) {
    for (uint8_t y = 0; y < qr_buffer->width; y++) {
      ...
      if (sli_sidewalk_qr_code_is_module_pixel_dark(qr_buffer, x, y)) {
        GLIB_drawRectFilled(&disp_hnd->glibContext, &rect);
      }
    }
  }
...
static void handle_status_msg(display_t *disp_hnd, const sl_sidewalk_display_msg_t *message)
{
  char stat_str_buff[24];

  if (!disp_hnd || !message) {
    return;
  }

  memset(stat_str_buff, 0, sizeof(stat_str_buff));

  snprintf(stat_str_buff, sizeof(stat_str_buff), "registration: %d", message->stats.is_registered);
  memset(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE][0], 0, SL_SIDEWALK_DISPLAY_CHAR_W_PX);
  memcpy(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE][0], stat_str_buff, strlen(stat_str_buff));

  snprintf(stat_str_buff, sizeof(stat_str_buff), "time sync: %d", message->stats.is_time_synced);
  memset(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE + 1][0], 0, SL_SIDEWALK_DISPLAY_CHAR_W_PX);
  memcpy(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE + 1][0], stat_str_buff, strlen(stat_str_buff));

  snprintf(stat_str_buff, sizeof(stat_str_buff), "link type: %d", message->stats.link);
  memset(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE + 2][0], 0, SL_SIDEWALK_DISPLAY_CHAR_W_PX);
  memcpy(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE + 2][0], stat_str_buff, strlen(stat_str_buff));
  snprintf(stat_str_buff, sizeof(stat_str_buff), "last tx msg id: %d", message->stats.last_successful_tx_seq_num);
  memset(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE + 3][0], 0, SL_SIDEWALK_DISPLAY_CHAR_W_PX);
  memcpy(&disp_hnd->buffer[SL_SIDEWALK_DISPLAY_STATUS_START_LINE + 3][0], stat_str_buff, strlen(stat_str_buff));
}
...
static void handle_normal_msg(display_t *disp_hnd, const sl_sidewalk_display_msg_t *message)
{
  ...
  msg_len = (int16_t)strlen(message->text);
  ...
  for (uint8_t start_line = SL_SIDEWALK_DISPLAY_MESSAGE_START_LINE; start_line < SL_SIDEWALK_DISPLAY_STATUS_START_LINE; start_line++) {
    memset(&disp_hnd->buffer[start_line][0], 0, SL_SIDEWALK_DISPLAY_CHAR_W_PX);
  }
  ...
  while (msg_len > 0) {
    ...
    memcpy(&disp_hnd->buffer[current_line][0], &message->text[msg_ix], chunk_to_copy);
