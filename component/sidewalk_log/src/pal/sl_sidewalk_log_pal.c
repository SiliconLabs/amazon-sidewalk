...
void sl_sid_log_pal(sl_sid_log_core_log_severity_t severity, const char *fmt, ...)
{
  ...
  sl_sid_log_core_set_rtt_terminal_id(SL_SIDEWALK_LOG_PAL_RTT_DEFAULT_TERMINAL_ID);
#endif
...
{
  if (severity <= SL_SID_LOG_PAL_SEVERITY) {
    char const digit[16] __attribute__((nonstring)) = "0123456789ABCDEF";
    uint8_t idx = 0;
    char hex_buf[LOG_MAX_HEXDUMP_CHAR * 3 + 1] = { 0 };
    const uint8_t *data = (const uint8_t *)address;
    for (int i = 0; i < length; i++) {
      if (idx && ((i % LOG_MAX_HEXDUMP_CHAR) == 0)) {
        SL_SID_LOG_PAL(severity, "%s", hex_buf);
        idx = 0;
      }
...
static sl_iostream_t *get_iostream(void)
{
  ...
  iostream = sl_iostream_get_handle(SL_SID_LOG_PAL_VCOM_INSTANCE_NAME);
#else
  iostream = NULL;
#endif
  return iostream;
