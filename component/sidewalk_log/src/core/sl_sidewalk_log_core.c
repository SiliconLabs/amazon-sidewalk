...
static void sli_sid_log_print_fmt(sl_sid_log_core_log_severity_t severity,
                                  const char *buffer);
// -----------------------------------------------------------------------------
//                                Global Variables
// -----------------------------------------------------------------------------
...
void sl_sid_log_core_print(sl_sid_log_core_log_severity_t severity,
                           const char *fmt,
                           va_list args)
{
  char buffer[LOG_MAX_BUFFER_CHAR] = { 0 };
...
{
  app_log_append("[%08u]" APP_LOG_SEPARATOR, (unsigned)get_time_now());
}
// -----------------------------------------------------------------------------
                                  ...
                                  const char *buffer)
{
  switch (severity) {
    case SL_SID_LOG_CORE_SEVERITY_ERROR:
      app_log_error("%s", buffer);
      break;

    case SL_SID_LOG_CORE_SEVERITY_WARNING:
      app_log_warning("%s", buffer);
      break;
