 ...
 *****************************************************************************/
static void help_print_and_pad(const char *permission,
 ...
 *****************************************************************************/
SL_WEAK const sl_sidewalk_cli_util_entry_t app_settings_entries[] = {
  { .key = NULL }
};
...
void sl_sidewalk_cli_util_settings_init(void)
{
  ...
  while (saving_settings[index]) {
    ...
    if (ret != SL_STATUS_OK
        && saving_settings[index]->default_val) {
      // if load did not succeed, set to default value
      memcpy(saving_settings[index]->data,
             saving_settings[index]->default_val,
             saving_settings[index]->data_size);
    }
    index++;
    ...
    if (!strcmp(domain, app_settings_domain_str[app_settings_entries[index].domain])) {
      if (!strcmp(app_settings_entries[index].key, key)) {
        if (app_settings_entries[index].set_handler) {
          SL_SID_LOG_APP_INFO("%s.%s = %s",
                              app_settings_domain_str[app_settings_entries[index].domain],
                              app_settings_entries[index].key,
                              value_str);
          return app_settings_entries[index].set_handler(value_str,
                                                         nested_key,
                                                         &app_settings_entries[index]);
        } else {
    ...
    if (!domain || !strcmp(domain, app_settings_domain_str[app_settings_entries[index].domain])) {
      if (!key || !strcmp(app_settings_entries[index].key, key)) {
        if (app_settings_entries[index].get_handler) {
          ret = app_settings_entries[index].get_handler(value_str,
                                                        nested_key,
                                                        &app_settings_entries[index]);
          if (ret == SL_STATUS_OK) {
            SL_SID_LOG_APP_INFO("%s.%s = %s",
...
sl_status_t sl_sidewalk_cli_util_help(char *const domain_and_key, bool get)
{
  ...
  const char *domain = NULL;
  char *key = NULL;
  bool key_and_domain_found = false;
  ...
  if (!domain) {
    SL_SID_LOG_APP_INFO("Help of get and set methods");
    SL_SID_LOG_APP_INFO("Available domains:");
    uint8_t domains_nb = 0;
    while (app_settings_domain_str[domains_nb] != NULL) {
      SL_SID_LOG_APP_INFO(" %s", app_settings_domain_str[domains_nb]);
      domains_nb++;
    }
    SL_SID_LOG_APP_INFO("Type '[get or set] domain help'");
    SL_SID_LOG_APP_INFO("eg. 'get %s help' to get the help of %s\r\n    'get %s' for all 'get' commands",
                        app_settings_domain_str[0], app_settings_domain_str[0], app_settings_domain_str[0]);
    SL_SID_LOG_APP_INFO("Commands permissions :\r\n %s : Write Only\r\n %s : Read Only\r\n %s : Read and Write",
                        permission_str[1], permission_str[2], permission_str[3]);
    key_and_domain_found = true;
  } else {
    while (app_settings_entries[index].key) {
      bool print_help = false;
      if (!strcmp(domain, app_settings_domain_str[app_settings_entries[index].domain])
          && (!key || !strcmp(app_settings_entries[index].key, key))) {
        print_help = true;
        key_and_domain_found = true;
      }

      permission |= (app_settings_entries[index].get_handler ? 0x02 : 0);
      permission |= (app_settings_entries[index].set_handler ? 0x01 : 0);

      if (print_help &&
          ((app_settings_entries[index].get_handler && get) ||
          (app_settings_entries[index].set_handler && !get))) {
                                            ...
                                            const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (strlen(value_str) >= entry->value_size) {
    SL_SID_LOG_APP_WARNING("String is too long, truncated");
  }
                                             ...
                                             const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  if (sl_sidewalk_cli_util_get_integer(&value,
                                       value_str,
                                       entry->input_enum_list,
                                       entry->input & SL_APP_SETTINGS_INPUT_FLAG_SIGNED) != SL_STATUS_OK) {
                               ...
                               const sl_sidewalk_cli_util_entry_t *entry)
{
  ...
  int ret = snprintf(formatted_string,
  ...
  if (ret >= (int)sizeof(formatted_string)) {
    SL_SID_LOG_APP_ERROR("Buff too small for help str, truncated");
  }
  SL_SID_LOG_APP_INFO("%-60s%s",
                      formatted_string,
                                     ...
                                     size_t settings_size)
{
  ...
  if (nvm3_getObjectInfo(nvm3_defaultHandle,
                         nvm_key,
                         &nvm_type,
                         &nvm_size) != ECODE_NVM3_OK) {
                                     ...
                                     size_t settings_size)
{
  ...
  if (nvm3_writeData(nvm3_defaultHandle,
                     nvm_key,
                     settings,
                     settings_size) != ECODE_NVM3_OK) {
    return SL_STATUS_FAIL;
  }
