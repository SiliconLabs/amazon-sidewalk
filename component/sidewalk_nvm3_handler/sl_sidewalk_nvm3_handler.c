                               ...
                               uint16_t length)
{
  ...
  if (nvm3_writeData(nvm3_defaultHandle, value, buffer, (size_t)length) != ECODE_NVM3_OK) {
    return SL_SIDEWALK_NVM3_DATA_WRITE_FAILED;
  }

  // Do repacking if needed
  if (nvm3_repackNeeded(nvm3_defaultHandle)
      && nvm3_repack(nvm3_defaultHandle) != ECODE_NVM3_OK) {
    return SL_SIDEWALK_NVM3_DATA_REPACK_FAILED;
  }

  return SL_SIDEWALK_NVM3_SUCCESS;
}
...
int32_t sl_sidewalk_nvm3_read(uint32_t value,
                              uint8_t *buffer)
{
  int32_t retVal = SL_SIDEWALK_NVM3_SUCCESS;
...
/*******************************************************************************
 *  Check if an object is valid in NVM3
 *
 *  @param[in]  value  A 20-bit object identifier
 *
 *  @return 1 if object exists
 *          0 if object is not found
 *          negative value on failure
 ******************************************************************************/
int32_t sl_sidewalk_nvm3_is_valid_object(uint32_t value)
{
  uint32_t object_type;
  size_t dataLen;
  int32_t retVal = 0;
  ...
  if (ret == ECODE_NVM3_OK) {
    retVal = 1;
  } else if (ret == ECODE_NVM3_ERR_KEY_NOT_FOUND) {
    retVal = 0;
  } else {
    retVal = SL_SIDEWALK_NVM3_GET_OBJECT_INFO_FAILED;
  }
