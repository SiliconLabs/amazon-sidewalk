...
static void generate_qr_code(uint8_t *qr_buffer, const char *text_to_encode);
...
void sli_sidewalk_qr_code_create_qr_from_str(sli_sidewalk_qr_code_qr_t *qr_buffer, char *string)
{
  if (qr_buffer != NULL && string != NULL) {
    ...
    qr_buffer->width = (uint8_t)qrcodegen_getSize(qr_buffer->buffer);
  }
}
...
{
  if (qr_buffer != NULL
      && qrcodegen_getModule(qr_buffer->buffer, x, y) == true) {
    return true;
  }
